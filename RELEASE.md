...
|---|---|
| **Brain / SPA** | **8.1.0** |
| **Expected firmware** | **8.1.0.0** _(tip `6f1b1fa` train bump — reflash for air-temp band entities + aligned fleet chip)_ |
...
**Tip `6f1b1fa` (2026-09-08 followups merge):** product firmware train **8.1.0.0**; Climate
honest air/efficacy + photoperiod re-anchor; `:6052` prefers `esphome-device-builder`; bake
uses declared `DEVDIR`. Published GitHub `v8.1.0` SD image may still ship **8.0.0.0** kit bins
until a rebake — OTA/reflash on the Pi is the path to 8.1.0.0.
