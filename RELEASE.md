...
|---|---|
| **Brain / SPA** | **8.1.0** |
| **Expected firmware** | **8.1.0.0** _(aligned-card `89ddfa7` / tip `6a89efa` — kit bins + live fleet)_ |
...
**Tip `6a89efa` (2026-09-08 QA):** pot2 EC `0` pre-watering is expected dry-medium physics, not a
fault — watering differential is the natural probe test ([TEST-PASS-8.1.0](docs/qa/TEST-PASS-8.1.0-2026-09-08.md);
runbook [`docs/ops/SOFT-CAL.md`](docs/ops/SOFT-CAL.md)). Product/firmware train unchanged from aligned-card.
...
`esphome-device-builder`; bake uses declared `DEVDIR`. Prefer tip `6a89efa` for current SoT;
card provenance remains tip `89ddfa7`.
