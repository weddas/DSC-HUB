...
Hard refresh `http://10.42.0.1:8787/#/ops/home` (Ctrl+Shift+R). Tip SPA assets include `index-BJFHGKyp.js` (Phase E FleetState + dual-slug ingest).
...
## Fleet flash / island proof / soak

```powershell
# OTA train via Pi ESPHome (quote-safe seat list — tip f10ad40)
services/dsc-hub/pi/flash-fleet-700.ps1
# Resume one seat: services/dsc-hub/pi/flash-fleet-700.ps1 -Seats "heater"

services/dsc-hub/pi/island-proof.ps1
services/dsc-hub/pi/soak-check.ps1
```

Sonoff nodes need **both** `project.version` and text `Firmware Version` at **7.0.0.0** or the fleet chip stays on `5.2.0`. Pot Modbus on Pi ESPHome **2025.12.4** must not use `turnaround_time` (use `send_wait_time`). Full runbook: [`docs/qa/PI-APPLIANCE-7.0.md`](../../docs/qa/PI-APPLIANCE-7.0.md).

Pi layout: `/opt/dsc-hub` (compose), `/opt/dsc-hub-repo` (full repo for builds). Bootstrap creates both symlinks.

`brain/static/` is a deploy artifact (gitignored). `deploy-brain.ps1` builds the SPA into `spa-dist/`, packs it into the brain image or hot-patches `/app/static/`. Prefer **image-build** after ingest/map changes. Ops: [`docs/qa/PI-APPLIANCE-7.0.md`](../../docs/qa/PI-APPLIANCE-7.0.md) · Audit: [`docs/AUDIT-2026-08-26.md`](../../docs/AUDIT-2026-08-26.md) · Soak: [`docs/ops/SOAK-2026-08-26.md`](../../docs/ops/SOAK-2026-08-26.md).
...
Tag `v7.0.0` after island proof + soak sign-off ([`RELEASE.md`](../../RELEASE.md)).
