...
Compare tip manifests (bytes prove payload weight; firmware train is from the
commit message + live QA, not a field in the JSON):

| Cut | Tip | `payload_bytes` | Meaning |
|---|---|---|---|
| 8.0.0 | hollow bake | **38 069** | Kit `.bin` placeholders / tiny payload |
| 8.1.0 morning | `cce5c74` | **4 798 652** | Real `DSC_RELEASE=1` bake — still **8.0.0.0** kit train / ESPHome 2026.6.5 |
| 8.1.0 aligned-card | **`89ddfa7`** (SoT tip **`6a89efa`**) | **4 820 089** | Rebake: kit **8.1.0.0** / ESPHome **2026.8.2**, cannalib, eight non-zero bins; tip `6a89efa` = dry-EC QA only |

Also committed: `deploy/dsc-hub-8.1.0-sd-manifest.json` (`built_at` 2026-09-08
**20:18** on tip `89ddfa7`). Live proof:
[`docs/qa/TEST-PASS-8.1.0-2026-09-08.md`](../../../docs/qa/TEST-PASS-8.1.0-2026-09-08.md).
...
tip `6f1b1fa` — source train **8.1.0.0** + device-builder preference; tip **`89ddfa7`** (rebake) / tip **`6a89efa`** (current SoT) —
aligned-card rebake so the SD matches that train. Do **not** flash AlphaPi.

**Trap — root bake leaves root-owned trees (fixed tip `502bbcb` / `521e416`):**
`bake-on-linux.sh` must run as root for docker, but the ESPHome build service runs
as `dsc`. Without the EXIT trap, root-owned PlatformIO / `.esphome` /
`.device-builder*` break `:6052` (`PermissionError` on the lock; ~4 s compile
deaths), and root-owned `/home/dsc/.docker` kills the **next** bake on
`buildx/.lock` after all eight binaries compile. The trap `chown`s those paths
back to `${DSC_SERVICE_USER:-dsc}` on every exit. Detail:
