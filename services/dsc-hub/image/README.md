...
`esp_http_client` into esp-idf SoftAP builds (panel/probes); tip `03743b8` takes the image
from ESPHome's reported **Build path** (cached SUCCESS no longer fails “no image”).
