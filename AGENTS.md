...
- Pi brain/SPA hotpatch from Windows uses PuTTY `plink`/`pscp` with `-batch -hostkey`, not OpenSSH `scp` (password prompt hangs in agent shells); prefer docker `kill`+`start` over `restart` (restart has hung the Pi); see `.audit/stress-*-hotpatch.ps1` and `dsc-pi-hotpatch.mdc`.
