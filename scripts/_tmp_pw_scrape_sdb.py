...
PROFILE = Path(os.environ.get("LOCALAPPDATA", "")) / "Temp" / "dsc-chrome-fresh-pw"
import os
...
def main() -> int:
    ...
    with sync_playwright() as p:
        ...
        if "just a moment" in title.lower() or "cloudflare" in (page.content()[:2000].lower()):
        ...
        # export cookies to jar for record
        ...
        for idx, url in enumerate(urls, 1):
            ...
            try:
                ...
                html = page.content()
                low = html[:3000].lower()
                if "just a moment" in low or (page.title().lower().find("just a moment") >= 0):
                    print(f"STOP CLOUDFLARE {url}")
                    break
                if "anubis" in low and "challenge" in low:
        ...
        ck.data["cursor"] = urls[min(len(urls), scraped) - 1] if scraped else ck.data.get("cursor")
        ck.save()
        sdb.write_partial(
            items,
    ...
    print(f"DONE scraped_n={scraped} dump_items={len(items)} mermaid={mermaid_n}")
