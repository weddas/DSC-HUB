...
**Dashboard 7.0 (`335ddc9`):** React `/dsc-hub` IA is Live/Grow/Tune/Fleet with
Twin keep-alive hosting `custom:dsc-the-dash-card`. The Lit Twin/Dash card still
ships here (HACS / `/local`); the React shell + Plant Seat drawers + honesty rail
are **not** in this HACS bundle — they ship via `custom_components/dsc_hub`
...
flowchart LR
  ...
  sync --> local["/local"]
  local --> twinKA["TwinKeepAlive host"]
  hacs --> twinKA
```
