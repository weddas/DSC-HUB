...
**Dashboard 6.3.0 (`c0d9ebe` → HACS sync `dae4522`):** Dash glass HUD callouts /
Build glow live in `homeassistant/www/`. After that lands on master,
**HACS → DSC-HUB System Map → Redownload** + hard-refresh, or rely on Sync www
concat. The React `/dsc-hub` panel (operable charts, Full Auto controls, Plant
Seat, Want editors) is **not** in this HACS bundle — it ships via
`custom_components/dsc_hub` (see
...
| React `/dsc-hub` panel + Plant Seat + Want/Full Auto | Sync / `ha-sync` → `custom_components/dsc_hub` |
