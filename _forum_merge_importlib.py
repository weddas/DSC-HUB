...
def main() -> int:
    ...
    spec = importlib.util.spec_from_file_location(
        "mst", str(ROOT / "scripts" / "merge_staging_to_master.py")
    )
    mst = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mst)
    L("merge module loaded")
    ...
    for fam in FAMILIES:
        verify[fam] = {
            "source_record": master.execute(
                "SELECT COUNT(*) FROM source_record WHERE id=?", (fam,)
            ).fetchone()[0],
            "grow_trait": master.execute(
                "SELECT COUNT(*) FROM grow_trait WHERE source_id=?", (fam,)
            ).fetchone()[0],
            "strain_variant": master.execute(
                "SELECT COUNT(*) FROM strain_variant WHERE source_id=?", (fam,)
            ).fetchone()[0],
            "chemistry_profile": master.execute(
                "SELECT COUNT(*) FROM chemistry_profile WHERE source_id=?", (fam,)
            ).fetchone()[0],
        }
    master.close()
    out = {
