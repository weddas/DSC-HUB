...
| `dsc_brain/hub_controls.py` | Hub entity_id ↔ object_id maps (legacy C++ id + ESPHome 2026 name-slug) |
...
| `dsc_brain/computed_ops.py` / `dash_computed.py` | `hass_extras` for SPA (`GET /fleet/computed`) |
...
| `dsc_brain/esphome_client.py` | Fleet Native API ingest (`hub.values` vitals / `controls` / `binaries` / text + grow-log events) |
...
| `dsc_brain/api.py` | FastAPI + SPA surface (`/fleet`, `/fleet/computed`, `/control/*`, `/history`, `/grow-log`) |
| `scripts/audit_hub_ingest.py` | Live hub OID coverage vs ingest maps (`--critical-only`) |
