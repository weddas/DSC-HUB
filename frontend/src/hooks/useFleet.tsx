...
export interface FleetContextValue {
  ...
  /** Pi synthetic / computed HA states (helpers, extras) carried alongside the fleet snapshot. */
  hassStates: Record<string, HassEntity>;
}

const FleetStoreContext = createContext<Store<FleetContextValue> | null>(null);

const EMPTY_HASS_STATES: Record<string, HassEntity> = {};
...
export function FleetProvider({
  ...
  useLayoutEffect(() => {
    ...
    // Skip full replace when the parsed fleet + hassStates are structurally identical
    // (WS may republish unchanged snapshots). Still advance tick/heartbeat metadata.
    if (
      deepEqual(prev.fleet, value.fleet) &&
      deepEqual(prev.hassStates, value.hassStates) &&
      prev.loading === value.loading &&
      prev.error === value.error &&
      prev.source === value.source
    ) {
      if (prev.lastUpdatedAt !== value.lastUpdatedAt || prev.tick !== value.tick) {
        store.setState({ ...prev, tick: value.tick, lastUpdatedAt: value.lastUpdatedAt, refresh: value.refresh });
      }
      return;
    }
...
export function useFleetStore(): Store<FleetContextValue> {
...
function selectSeat(fleet: FleetSnapshot, seatId: string): SeatSnapshot {
  ...
  if (seatId.startsWith("pot"))
    return (
      fleet.pots[seatId] ??
      EMPTY_FLEET.pots[seatId] ?? {
        seat_id: seatId,
        online: false,
        firmware: null,
        values: {},
...
/** Per-entity control slice — EntityToggle / FanSlider path. */
export function useFleetControlSlice(entityId: string) {
  return useFleetSelector((v) => {
    const state = fleetControlState(entityId, v.fleet);
    if (state == null) return null;
    return { state, attributes: fleetControlAttributes(entityId, v.fleet), online: v.fleet.hub.online };
  }, deepEqual);
}
