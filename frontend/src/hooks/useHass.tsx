...
export function HassProvider({
  ...
  revision?: number;
  /**
   * When false (Pi SPA), revision does not bump context tick — per-entity fleet
   * selectors / useEntityBus own re-renders. hassRef still tracks props + piHassBridge.
   */
  ...
  useLayoutEffect(() => {
    hassRef.current = hass ?? piHassBridge.current;
  }, [hass]);
  const conn = (hass ?? piHassBridge.current)?.connection;
  const hassPresent = !!(hass ?? piHassBridge.current);
  ...
  // On Pi with fleet selectors, skip — mass useHass re-renders are the whole-fleet bug.
  useEffect(() => {
    if (revisionDrivesTick && revision > 0) bumpTick();
  }, [revision, revisionDrivesTick]);
  ...
  const liveHass = () => hassRef.current ?? piHassBridge.current;
  ...
  const value = useMemo<HassContextValue>(() => {
    const entity = (entityId: string) => liveHass()?.states?.[entityId];
    ...
    return { hass: liveHass(), entity, state, num, available, callService, callWS, tick };
