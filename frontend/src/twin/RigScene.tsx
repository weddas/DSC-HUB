      ...
      <Placed id="room" slug="grow-room-shell" at={{ position: [0, 0, 0] }} bind={{ tint: [{ material: "trim_green", tone: toneOf(state.zones.room.tone) }] }}>
        ...
        >
          ...
          {layers.devices ? (
            <>
              ...
              {/* The puck's LED follows the reading it publishes, not the hub's heartbeat. */}
              <Placed id="puck4x8" slug="sensor-zigbee-puck" at={{ parent: "tent4x8", anchor: "canopy_sensor" }} self="hang_point" bind={{ emissive: [{ node: "puck_led", on: main.temp.available && !main.temp.stale, tone: "ok", level: 0.6 }] }} pick={{ entityId: main.temp.entityId, label: "4×8 air temperature", unit: main.temp.unit }} />
        ...
        >
          ...
          {layers.devices ? (
            <>
              ...
              <Placed id="puck2x4" slug="sensor-zigbee-puck" at={{ parent: "tent2x4", anchor: "canopy_sensor", offset: [0, 0.13, 0] }} self="hang_point" bind={{ emissive: [{ node: "puck_led", on: clone.temp.available && !clone.temp.stale, tone: "ok", level: 0.6 }] }} pick={{ entityId: clone.temp.entityId, label: "2×4 air temperature", unit: clone.temp.unit }} />
        ...
        {layers.devices ? (
          <>
            ...
            {/* No level or temperature sensor in the tank — it is declared unbound and drawn as scenery. */}
            <Placed id="tank" slug="reservoir-tank-60l" at={{ parent: "room", anchor: "tank_spot" }} />
            <Placed
              id="brain"
              slug="brain-rpi"
              at={{ parent: "room", anchor: "tank_spot", offset: [0, 0.36, 0] }}
              bind={{
                // The brain is only *seen* through the fleet it serves: no heartbeat, no LEDs.
                emissive: [
                  { node: /^brain_(power|activity)_led$/, on: state.hubOnline, tone: "ok", level: 0.9 },
                  { node: "zigbee_led", on: state.hubOnline, tone: "teal", level: 0.8 },
                ],
              }}
              pick={{ entityId: "sensor.dsc_hub_heartbeat", label: "Brain (Pi)" }}
            />
