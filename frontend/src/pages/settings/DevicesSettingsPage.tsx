        ...
        <>
          <section className="dsc-card" id="firmware" style={{ scrollMarginTop: 80 }}>
              ...
              {toolchain ? (
                <>
                  <p className="dsc-muted" style={{ margin: "4px 0" }}>
                    ...
                    {" · "}Pinned min <b>{String(toolchain.min_version ?? "—")}</b> {toolchain.meets_min === false ? <StatusChip label="BELOW PINNED MIN" tone="bad" /> : null}
                  </p>
                  {toolchain.latest_ok === false ? (
                    <p className="dsc-muted" style={{ margin: "4px 0", fontSize: "var(--dsc-fs-sm)" }} title={toolchain.latest_error ? String(toolchain.latest_error) : undefined}>
                      {toolchain.eth_up === false ? "Can't check for a newer ESPHome — the Pi has no network uplink." : "Can't reach PyPI to check for a newer ESPHome (no internet / DNS). Installed + pinned checks still work."}
                    </p>
                  ) : null}
                  <p style={{ margin: "6px 0", display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
                    <Button onClick={() => setPendingToolchain(true)} disabled={toolchain.update_available !== true || toolchain.eth_up !== true}>
                  ...
                  {toolchain.build_backend === "dashboard" ? (
                    <p style={{ margin: "4px 0", fontSize: "var(--dsc-fs-sm)" }}>
                      <StatusChip label="Deprecated backend" tone="warn" /> The <code>dsc-hub-esphome</code> container backend is being retired in 8.x — the host ESPHome venv is the supported path.
                    </p>
                  ) : null}
                  {Array.isArray(toolchain.devices_behind) && (toolchain.devices_behind as string[]).length > 0 ? (
                    <p className="dsc-muted" style={{ margin: "4px 0" }}>
                      Running an ESPHome other than {String(toolchain.installed ?? "installed")}: {(toolchain.devices_behind as string[]).join(", ")}
                    </p>
                  ) : null}
                  ...
                  {rollout && rollout.needed === true && Array.isArray(rollout.seats) && (rollout.seats as unknown[]).length > 0 ? (
                    <p className="dsc-honesty" style={{ margin: "6px 0" }}>
                      Toolchain changed — {(rollout.seats as unknown[]).length} device(s) can be reflashed on the new ESPHome (serialised, hub last). <Button onClick={() => setPendingRollout(true)}>Reflash fleet</Button>
                    </p>
                  ) : null}
                </>
              ) : (
            ...
            <DecisionLayer
              ...
              onConfirm={async () => {
                ...
                try {
                  ...
                  if (r && r.status === "manual") setToolchainMsg(String(r.detail ?? "Redeploy the ESPHome container on the Pi."));
                  else setToolchainMsg(`Update started → ${String(r.target ?? "latest")}. Watch the log below.`);
                } catch (e) {
            ...
            >
              {toolchain?.build_backend === "dashboard" ? (
                <p>
                  Bumps <code>image: esphome/esphome</code> to <b>{String(toolchain?.latest ?? "latest")}</b> in <code>{String(toolchain?.compose_file ?? "docker-compose.yml")}</code> and redeploys the <code>esphome</code> service. No devices are touched. After it finishes you&apos;ll be offered a fleet reflash.
                </p>
              ) : (
                <p>
                  Runs <code>pip install -U esphome</code> in the Pi venv, then restarts the dashboard service. No devices are touched. After it finishes you&apos;ll be offered a fleet reflash.
                </p>
              )}
            </DecisionLayer>
            <DecisionLayer
            ...
            >
              <p>
                Enqueues one OTA per in-service device{Array.isArray(rollout?.seats) ? ` (${(rollout!.seats as unknown[]).length})` : ""}. Jobs run serialised through the existing build worker, with the hub flashed last.
              </p>
            </DecisionLayer>
