...
import { inventoryGroup, resolveSeat, taskParamDefaults } from "../../components/settings/settingsHelpers";
...
/**
 * Settings › Devices — inventory, assignment, probe stations, Zigbee, cameras and firmware.
 * Pass S1 moves the cards out of the old single-file SettingsPage unchanged in behaviour;
 * the drawer-based sub-tabs (Inventory / Assignment / Zigbee / Cameras / Firmware) are
 * Pass S5. Cameras (S7) already use the drawer pattern (`CamerasCard`).
 * Anchors: #inventory #assignment #probe-stations #zigbee #cameras #firmware.
 */
...
export function DevicesSettingsPage() {
  ...
  const [zigbeeBindDraft, setZigbeeBindDraft] = useState<Record<string, BindDraft>>({});
  const [zigbeeShowAll, setZigbeeShowAll] = useState<Record<string, boolean>>({});
  const [zigbeePolicyDraft, setZigbeePolicyDraft] = useState<Record<string, PolicyDraft>>({});
  const [zigbeeBindDirty, setZigbeeBindDirty] = useState(false);
  const [zigbeeSaving, setZigbeeSaving] = useState(false);
  ...
  const refresh = async () => {
    const [s, esp, j, fleetRaw, zigbee, zigbeeHealthRaw, stations, rolesRaw, recipesRaw, policiesRaw] =
      await Promise.all([
        get_settings(),
    ...
    if (!zigbeeBindDirty) {
      ...
      setZigbeeBindDraft(drafts.bind);
      setZigbeePolicyDraft(drafts.policy);
    }
  ...
  return (
    <>
      {/* ---- Inventory ---------------------------------------------------- */}
      <section className="dsc-card" id="inventory" style={{ scrollMarginTop: 80 }}>
          ...
          <div className="dsc-row-actions" style={{ flexWrap: "wrap", gap: 12, alignItems: "flex-end" }}>
            ...
            <label>
              ...
              <select
                ...
                onChange={(e) => setAddSeatDraft((d) => ({ ...d, kind: e.target.value as "sensor" | "appliance" }))}
              >
        ...
        >
          <p>
            Creates an inventory row for <strong>{addSeatDraft.name || addSeatDraft.seat_id}</strong> ({addSeatDraft.kind},{" "}
            {addSeatDraft.placement}). It starts <b>out of service</b> and fakes no readings until you enable it.
          </p>
        </DecisionLayer>
                ...
                <div key={String(row.seat_id)} className="dsc-col-4">
                  ...
                  <label style={{ display: "block", marginTop: 8, fontSize: "0.85rem" }}>
                    <input
                      ...
                      onChange={(e) => setPendingInService({ seatId: String(row.seat_id), next: e.target.checked })}
                    />{" "}
                    In service
                  </label>
                </div>
                      ...
                      <tr key={st.seat_id}>
                        ...
                        <td>
                          <select
                            ...
                            onChange={(e) =>
                              setProbeDrafts((prev) => ({ ...prev, [st.seat_id]: { ...draft, idle_home_pot_id: e.target.value } }))
                            }
                          >
                        ...
                        <td>
                          <select
                            value={draft.tent}
                            onChange={(e) =>
                              setProbeDrafts((prev) => ({ ...prev, [st.seat_id]: { ...draft, tent: e.target.value } }))
                            }
                          >
                            {TENT_OPTIONS.map((t) => (
      ...
      {/* ---- Zigbee ------------------------------------------------------- */}
      <div id="zigbee" style={{ scrollMarginTop: 80 }}>
        ...
        {zigbeeDevices.filter((d) => d.type !== "Coordinator").length ? (
          <>
            <p className="dsc-muted" style={{ marginTop: 12 }}>
              New devices appear after permit join. Assign a <strong>Role</strong>, <strong>Zone</strong>, and optional{" "}
              <strong>Task</strong> — lists are filtered by device type; use <strong>Show all</strong> for
              mis-fingerprinted sensors (e.g. occupancy-only liquid probes).
            </p>
            <p className="dsc-muted" style={{ marginTop: 8, fontSize: "var(--dsc-fs-md)" }}>
              Role is where this sensor lives (intake, canopy, tank…). Task is optional — leave <strong>No task</strong> to
              only report into Live/Climate.
            </p>
            ...
            <SettingsTable
              ...
              help={{
                ...
                body: (
                  <>
                    <p>
                      Click a device name to rename it — the alias is what Climate and Overview show. Clearing it falls
                      back to the raw Zigbee name.
                    </p>
                    <p>
                      <b>Health</b> is battery, link quality (LQI) and last-seen straight from the device&apos;s own
                      reports; amber means low battery, weak link, or silent for over an hour.
                    </p>
                  </>
                ),
              }}
            >
              {zigbeeDevices
                ...
                .map((d) => {
                  ...
                  const policy = zigbeePolicyDraft[ieee] ?? { recipe_id: "none", enabled: true, params: {} };
                  ...
                  const zigbeeByRoleLive = (fleet?.system?.zigbee_by_role ?? fleet?.system?.zigbee_by_placement ?? {}) as Record<
                    string,
                    { wet?: boolean; active?: boolean }
                  >;
                  const roleLive = zigbeeByRoleLive[draft.role];
                  const liveWet =
                    typeof roleLive?.wet === "boolean" ? roleLive.wet : typeof roleLive?.active === "boolean" ? roleLive.active : null;
                  ...
                  return (
                    <ZigbeeBindRow
                      ...
                      onToggleShowAll={() => setZigbeeShowAll((prev) => ({ ...prev, [ieee]: !prev[ieee] }))}
                      ...
                      onPolicyChange={(id, patch) => {
                        ...
                        setZigbeePolicyDraft((prev) => ({
                          ...prev,
                          [id]: { recipe_id: patch.recipe_id, enabled: true, params: patch.params },
                        }));
                      }}
                    />
                  );
            ...
            <Button
              onClick={async () => {
                ...
                try {
                  ...
                  setZigbeeSaving(false);
                }
              }}
              disabled={!zigbeeBindDirty || zigbeeSaving}
            >
              Save roles &amp; tasks
            </Button>
          </>
          ...
          <div className="dsc-muted" style={{ marginTop: 10, fontSize: "var(--dsc-fs-md)", lineHeight: 1.45 }}>
            <p style={{ margin: "0 0 8px" }}>
              Coordinator online — no end devices yet. Role/Zone rows appear here as soon as a sensor joins (this page
              polls while JOIN OPEN).
            </p>
            <ol style={{ margin: 0, paddingLeft: 18 }}>
              <li>Confirm JOIN OPEN (or tap Permit join ~4 min). Keep house ZHA pairing closed so the sensor joins this SkyConnect.</li>
              <li>
                Factory-reset a TS0201 (hold reset ~5s until LED blinks). Leaving ZHA alone is not enough — hold reset
                near this Pi. Freed grow sensors: Canopy Middle/Left/Right, Tent Top, Floor, Tent Bottom. DSC network is
                new after Zigbee re-flash (channel 11).
              </li>
              <li>
                When the Unbound row appears, assign Role + Zone → Save roles (or wait for auto-bind). Climate / Overview
          ...
          <p className="dsc-muted" style={{ marginTop: 10 }}>
            SkyConnect coordinator is not online — fix USB, power, and <code>dsc-hub-z2m</code> logs before pairing. An
            empty device list here means the radio is down, not that you have a clean network.
          </p>
        )}
      </section>

      {/* ---- Firmware (ESPHome) — absorbs the old Server tab ---------------- */}
      {/* ---- Cameras (S7) -------------------------------------------------- */}
      <CamerasCard />
      ...
      <section className="dsc-card" id="firmware" style={{ scrollMarginTop: 80 }}>
        ...
        <p className="dsc-muted">
          Updates are sent over the air. One build runs at a time, and nothing is flashed unless you queue it. Probe 5 and
          beyond are unavailable until their firmware exists.
        </p>
          ...
          {toolchain ? (
            <>
              <p className="dsc-muted" style={{ margin: "4px 0" }}>
                ...
                {" · "}Latest <b>{toolchain.latest_ok ? String(toolchain.latest ?? "—") : "offline"}</b>
              ...
              {toolchain.build_backend === "dashboard" ? (
                <p style={{ margin: "4px 0", fontSize: "var(--dsc-fs-sm)" }}>
                  ...
                  is being retired in 8.x — the host ESPHome venv is the supported path. Switch when you can.
                </p>
              ) : null}
              ...
              {rollout && rollout.needed === true && Array.isArray(rollout.seats) && (rollout.seats as unknown[]).length > 0 ? (
                <p className="dsc-honesty" style={{ margin: "6px 0" }}>
                  Toolchain changed — {(rollout.seats as unknown[]).length} device(s) can be reflashed on the new ESPHome
                  (serialised, hub last). <Button onClick={() => setPendingRollout(true)}>Reflash fleet</Button>
                </p>
              ) : null}
            </>
          ) : (
            <p className="dsc-muted" style={{ margin: "4px 0" }}>
              Toolchain status unavailable — brain offline, or neither the ESPHome dashboard (:6052) nor a local venv is
              reachable.
            </p>
          )}
        ...
        <DecisionLayer
          ...
          onConfirm={async () => {
            ...
            setToolchainMsg(toolchain?.build_backend === "dashboard" ? "Bumping the ESPHome container image…" : "Updating ESPHome venv…");
            try {
        ...
        <h4 style={{ marginTop: 16 }}>Job history</h4>
        {jobs.length ? (
          <SettingsTable
            columns={[
            ...
            help={{
              title: "Job history",
              body: <p>Every ESPHome compile / OTA the build worker has run. Queue new jobs above; this is the record of what happened.</p>,
            }}
          >
            {[...jobs]
              ...
              .map((j, i) => {
                ...
                const tone: "ok" | "warn" | "bad" | "muted" | undefined =
                  status === "done" ? "ok" : status === "failed" ? "bad" : status === "running" || status === "queued" ? "warn" : "muted";
        ...
        <p className="dsc-honesty" style={{ marginTop: 10 }}>
          Full Update pull requires Ethernet (`GET/POST /settings/update`). Offline kits stay on the baked card version —
          no silent “updated” without matching digests. Kit USB flash + commission live under{" "}
          <a href={`#${paths.setup()}`}>Setup</a>.
        </p>
      </section>
