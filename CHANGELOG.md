...
- **Hub link Age + HelpTip** — `HubLinkLine` formats Age/Beat with `fmtUptimeSeconds`; inline `?` via native `<details>` (`HelpTip`). Closes DA-P1-1 in source. Docs: [`docs/brain/HELP-TIP.md`](docs/brain/HELP-TIP.md). **SPA rebuild** still required — committed `spa-dist` hash unchanged until next Vite pass.
...
- Tip `39d7f88` · SPA `index-DL1EcjhX` (+ `calibrate-D1D5CnxU` · `tune-fleet-IPnSFs3d`) until rebuild. Surface still **7.3.0** until Phase E.
