...
- **Research catalog corpus + HA projection (N-087)** — Wave A–C importers, SQLite science↔seed schema (`corpus_schema` / `corpus.py`), PPFD archival + gap report, community export stub, and slim `/local/dsc-catalog/` indexes from SQLite strain projection (cap 2500). HA surface **5.1.13**. Ops: [`docs/qa/CATALOG-RESEARCH-CORPUS.md`](docs/qa/CATALOG-RESEARCH-CORPUS.md).
- **Unified HA product shell (N-086)** — One sidebar **DSC-HUB** with Ops / Plant / Advanced / System; Catalog Explorer + Compose under Plant; nav + catalog cards in Sync/HACS concat. Surface **5.1.12** (superseded marker **5.1.13** with N-087). Ops: [`docs/qa/LIVE-UI-PRODUCT-SHELL.md`](docs/qa/LIVE-UI-PRODUCT-SHELL.md).
