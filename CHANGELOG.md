...
- **Hub link Age/Beat honesty + HelpTip** — `HubLinkLine` shows online→`Up` / offline→`Down` (`0` → `0S`); Beat is `HS` handshake duration **or** `HB #` heartbeat count (never format the tick as hours). Inline `?` via native `<details>` + opaque `modalLayer` symbols. Desk tips include Mission / Twin / cockpit + Climate Zone focus. Honesty rail: `+N` overflow, held Got, CFM nameplate → Learning, CTAs avoid demoted Mission. Closes DA-P1-1 in source. Docs: [`docs/brain/HELP-TIP.md`](docs/brain/HELP-TIP.md). **SPA rebuild** still required — committed `spa-dist` lacks `dsc-help-tip`.
...
- **Conditional hooks (mm-review Act-on)** — `usePanelOfflineMs` always calls `useOfflineMs`; `useHeldReading` clears hold **during render** on `entityId` change; `HassProvider` assigns `hassRef` **during render** (tip `8fc5e33`).
...
- Tip `8fc5e33` · SPA `index-DL1EcjhX` (+ `calibrate-D1D5CnxU` · `tune-fleet-IPnSFs3d`) until rebuild. Surface still **7.3.0** until Phase E.
