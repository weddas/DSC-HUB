...
- **DSC-Brain 7.0.0 island product + fleet flash (`f4299f6`…`f10ad40`)** — Phase E splits native `/fleet` from `/fleet/computed`; surface **7.0.0**; SPA `index-BJFHGKyp`; operator scripts `flash-fleet-700.ps1` / `island-proof` / `soak-check`; pot Modbus drops `turnaround_time` for ESPHome 2025.12.4; Windows seat-list quoting; Sonoff text `Firmware Version` → **7.0.0.0**. Ops: [`docs/qa/PI-APPLIANCE-7.0.md`](docs/qa/PI-APPLIANCE-7.0.md) · Soak: [`docs/ops/SOAK-2026-08-26.md`](docs/ops/SOAK-2026-08-26.md).
