...
- **Unified HA product shell (N-086)** — One sidebar **DSC-HUB** with primary **Ops · Plant · Advanced · System**; Build folded into Plant Compose; Catalog Explorer browse/compare/Use in Build; HA surface **5.1.12**. Sync/HACS www concat adds `dsc-app-nav-card` + `dsc-catalog-browse-card` (healthy bundle ~979 KB). Ops: [`docs/qa/LIVE-UI-PRODUCT-SHELL.md`](docs/qa/LIVE-UI-PRODUCT-SHELL.md).
- **Pi offline brain (Phase A/B start)** — Product destination documented in Notion [Product layers](https://app.notion.com/p/3b52b4cda37081c2bcafc85d3407556c) + [`docs/DSC-BRAIN.md`](docs/DSC-BRAIN.md). New `brain/` package loads curated catalogs into SQLite, resolves Want, dry-runs decision ticks, exposes FastAPI stub (`:8787`). Specs: [`docs/brain/`](docs/brain/). HA is lab scaffold ([`docs/HA-SCAFFOLD.md`](docs/HA-SCAFFOLD.md)); SoftAP remains product unbox. F-010 appliance bridge design + ESPHome sketch `firmware/v4/dsc-appliance-bridge.yaml`.
- **Build a Plant Full Inclusion (N-085)** — Pro Home/Root Zone roster context, Nutrient Science calculator/deep-link/CANNA stage packs, Lighting catalog controls, short-stock-safe Accept, refreshed catalog indexes, and HA surface **5.1.11** (superseded for shell layout by N-086 / **5.1.12**).
- **Build a Plant (N-083)** — Composition card + package `dsc_v4_build_plant.yaml` (soil % blend, plant roster, mix calculator, Apply climate Want). Originally a separate `dsc-build-plant` dashboard; **N-086** folds Compose under `/dsc-hub-pro/plant-build`. Slim `/local/dsc-catalog/` search indexes via `scripts/build_catalog_search_indexes.py`. Wired into `sync-hacs-dist.sh` / `ha-sync.sh` (bundle + catalog + dashboard copy).
- **Sync 5.1.4 (N-084)** — HAOS Sync add-on concatenates Build a Plant into www/`DSC-HUB.js`, stages `dsc-build-plant-dashboard.yaml` + `www/dsc-catalog/`. **N-086** extends concat with nav + Catalog Explorer. Rebuild add-on after Update (≤5.1.3 stopped at The Dash). Ops: `docs/qa/LIVE-UI-BUILD-A-PLANT.md` · `docs/qa/LIVE-UI-PRODUCT-SHELL.md`.
