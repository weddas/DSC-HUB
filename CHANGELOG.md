...
- **DSC-HUB React custom panel (surface 6.0.0)** — WashData-style HA built-in panel at `/dsc-hub` (`custom_components/dsc_hub`, Vite React + HashRouter). Sidebar **DSC-HUB** leaves Lovelace `dsc-hub-pro` as a hidden YAML fallback. Ops/Plant/Advanced/System tabs; native KPI pages + `LegacyCardHost` for The Dash / Build / Catalog / system-map. Sync + `ha-sync.sh` deploy Python/`www`/assets. Fleet expected helper initial **5.2.0**; optional bridge row on fleet chip. Ops: [`docs/qa/LIVE-UI-CUSTOM-PANEL.md`](docs/qa/LIVE-UI-CUSTOM-PANEL.md).
