...
- **Hub link Age/Beat honesty + HelpTip** — `HubLinkLine` shows online→`Up` / offline→`Down` (`0` → `0S`); Beat is `HS` handshake duration **or** `HB #` heartbeat count (never format the tick as hours). Inline `?` via native `<details>` + opaque `modalLayer` symbols. Desk tips include Mission / Twin / cockpit + Climate Zone focus. Honesty rail: `+N` overflow, OOS pot count → Root, held Got, CFM nameplate → Learning, CTAs avoid demoted Mission. Dash pot chips split ESP-NOW vs Modbus probe. Closes DA-P1-1. Docs: [`docs/brain/HELP-TIP.md`](docs/brain/HELP-TIP.md).
- **ZoneFocus URL ownership** — `setFocus` writes `?tent=` only on Climate / Dash; sync from URL only when `tent`/`zone` present; TentCockpit no longer fights Shell strip; `emphasize` dims Want columns. Docs: HELP-TIP § ZoneFocus.
- **Conditional hooks (mm-review Act-on)** — `usePanelOfflineMs` always calls `useOfflineMs`; `useHeldReading` clears hold **during render** on `entityId` change; `HassProvider` assigns `hassRef` **during render**.
...
- Tip `a50d402` · SPA `index-Ciw7XTuZ` (+ `calibrate-CzxOXAJy` · `tune-fleet-KoWKiPmD`). Surface still **7.3.0** until Phase E.
- Tip `7fa85b5` · Pi polish SPA hot-patched to `.48:8787` after C:→Y: spa-dist sync; Help 1.2.3 publish still blocked on WP tunnel/LAN; `Dockerfile.prebuilt` seed pack pitfall documented in [`docs/ops/DSC-HUB-DOCKER.md`](docs/ops/DSC-HUB-DOCKER.md).
