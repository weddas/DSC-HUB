...
(`6a89efa` SoT / `89ddfa7` aligned-card bake with kit **8.1.0.0** / ESPHome **2026.8.2**; published
...
## AlphaPi (`v8.0.0-AlphaPi`) → **8.1.0** (+ tip `6a89efa` / aligned-card `89ddfa7`)

**Brain / SPA first**, then decide on fleet OTA.

- Published `v8.1.0` card / AlphaPi→8.1.0 Kit Update is a **brain** bump.
- Tip `6f1b1fa` raised `EXPECTED_FIRMWARE` to **8.1.0.0**; tip **`89ddfa7`** (rebake) / tip **`6a89efa`** (QA EC)
  aligned-card rebake ships those kit bins on the SD (ESPHome **2026.8.2**).
  Kits still on **8.0.0.0** hub firmware show train drift until canary →
  release-the-rest (or USB reflash from the aligned card).
...
3. Offline / no Ethernet: pull tip `6a89efa` via workstation deploy or
...
3. Settings → Devices → Firmware → **Reflash whole fleet** (hub last). The 8.x
   train (tip `6a89efa` / aligned-card `89ddfa7` → **8.1.0.0**) drops every `platform: homeassistant`
