...
# Optional Nest STA join keeps ESPHome ESP-NOW on WIFI_IF_STA with the hub.
...
CONF_STA_SSID = "sta_ssid"
CONF_STA_PASSWORD = "sta_password"
...
CONFIG_SCHEMA = cv.Schema(
    {
        ...
        cv.Optional(CONF_STA_SSID): cv.string,
        cv.Optional(CONF_STA_PASSWORD): cv.string,
...
async def to_code(config):
    ...
    if CONF_STA_SSID in config:
        cg.add(var.set_sta_ssid(config[CONF_STA_SSID]))
    if CONF_STA_PASSWORD in config:
        cg.add(var.set_sta_password(config[CONF_STA_PASSWORD]))
