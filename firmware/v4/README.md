...
**Live train:** hub / Control / probe1+2 **7.0.0.0** (Climate Mode 0xD1 v2 + `dsc_probeN`;
surface **7.3.0**, 7.4 WiP). pot3/4 retired from kit (YAML kept). See CHANGELOG / FOLLOWUPS.
...
Device **names** are `dsc_probeN` (entity contract `sensor.dsc_probeN_*`). Live kit flashes **probe1 + probe2** only (pot3/4 planned OOS). Flash hub providers with in-kit probes in one train. Pi compile/OTA runbook (compose DNS pin + ESPHome 2025.12 Control path + API-key casing): [`../../docs/ops/ESPHOME-OTA-PI.md`](../../docs/ops/ESPHOME-OTA-PI.md). SoftCal / `cal_session`: [`../../docs/ops/SOFT-CAL.md`](../../docs/ops/SOFT-CAL.md).
