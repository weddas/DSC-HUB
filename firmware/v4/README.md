...
**Live train:** hub **5.1.7** Â· Control **5.1.15** Â· pots **5.1.6** (tagged marketing
...
Entry points (local lab): `dsc-hub.yaml`, `dsc-control.yaml`, `dsc-pot1.yaml`, â€¦
...
Pots (`dsc-pot-common` **5.1.6+**): each soil channel has **Cal â€¦ Offset** / **Cal â€¦ Scale**
...
| Feature | Notes |
|---|---|
| Soil cards + detail | 0xD3 vitals / 0xD4 names; tap pot â†’ NPK drill-down |
| Hold-to-lock | Hold ~3â€¯s on primary tabs; hold lock screen to unlock |
| Demand / takeover gate | Confirm â†’ Engage (not one stray tap) |
| Connections | Wiâ€‘Fi channel; ESP-NOW RX age + TX seq; silent â†’ ping/WiFi bounce |
| AP pin | Runtime only: hub **Lock WiFi AP** learns preferred BSSID into NVS; 0xD0 fleet-beats it; Control/pots `adopt_hub_wifi_ap`. Stubs stay `00:00:00:00:00:00` â€” never bake a site MAC into YAML. |
| Pulse VPD trend | 12Ã—5â€¯min ring â†’ one label (no canvas charts) |
| HA API | **Plaintext** (no Noise); **mDNS off** â€” add by IP only |
| Stability | **4.0.10** page-gated `refresh_ui` @ 5â€¯s |
| Snappiness | **4.0.11** `refresh_ui` reads `gv_*` live (template mirrors parked); 30â€¯s Wiâ€‘Fi channel poll |

After UI flashes: watch serial `boot` / `heap` lines. If the panel boot-loops, use **USB** not OTA until `DSC-CONTROL 4.0.11 up â€” free_heap=â€¦` prints cleanly. See [`../_history/v4/crash-logs/`](../_history/v4/crash-logs/).

### Panel HA API reconnect

The `api:` block lives in [`dsc-control-common.yaml`](dsc-control-common.yaml). **v4.0.9+ has no Noise encryption** â€” LVGL RAM left the Noise handshake failing (`HANDSHAKESTATE_SETUP_FAILED`) and the teardown path double-freed the heap (reboot whenever HA probed). mDNS is **disabled** (setup OOM left it FAILED forever). ESPHomeâ€™s â€œUnable to connectâ€¦ includes an `api` sectionâ€ toast is **generic** â€” it does **not** mean the YAML is missing `api:`.

| Check | What to do |
|---|---|
| Panel boot-looping / no Wiâ€‘Fi | USB flash; serial must show `DSC-CONTROL 4.0.11 up â€” free_heap=â€¦`. OTA will not recover a looping board. |
| Host / mDNS | **IP only** â€” lab Nest reservation **`192.168.86.177`** (`use_address` in `dsc-control-wifi-lab.yaml`). Do not use `dsc-control.local`. |
...
| Bundle fails: `â€¦ is not a valid YAML file` / `expected '<document start>'` | Almost always a **header comment** in the package body that lost its `#` (looks like `v4.0.x:` at column 2). ESPHome then treats the changelog line as YAML and dies before `substitutions:`. Fix on git, push, set stub `refresh: 0d`, Validate again. |

ESP-NOW (glass â†” hub) does **not** need the HA API. Fix API only for OTA, diagnostics, and HA time backup.
...
In [`dsc-hub-v4_0.yaml`](dsc-hub-v4_0.yaml): `Mat Vote Pot 1`â€“`4` (`switch.dsc_hub_mat_vote_pot_N`). OFF pots are skipped by coldest/hottest root-zone voting (5â€“45â€¯Â°C filter still applies). POT3 defaults OFF.
...
`espnow_cmd_tag` is **54727** (`0xD5C7`) on hub + panel â€” flash both after changing it.

Fleet bring-up / cutover: [`../../INSTALL.md`](../../INSTALL.md) Â· [`../../UPGRADE.md`](../../UPGRADE.md).
