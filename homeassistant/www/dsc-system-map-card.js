 ...
 * Resource: /local/dsc-system-map-card.js
 * SVG art:  /local/dsc-system-map.svg
 *
 * type: custom:dsc-system-map-card
 */
(() => {
  const CARD_TYPE = "dsc-system-map-card";
  const SVG_URL = "/local/dsc-system-map.svg";
  ...
  class DscSystemMapCard extends HTMLElement {
    ...
    async _loadSvg() {
      ...
      try {
        const res = await fetch(`${SVG_URL}?v=${Date.now()}`, { cache: "no-cache" });
        ...
        this._wrap.innerHTML = `<div style="color:#F44336;font-family:monospace;padding:24px;text-align:center;">
          ...
          Copy <code>dsc-system-map.svg</code> to <code>/config/www/</code><br/>
          <span style="color:#8B93A7;font-size:12px;">${String(err.message || err)}</span>
        </div>`;
        this._svgReady = false;
      }
    }
