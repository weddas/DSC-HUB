...
8. **Power the ETH01 bridge** (Ethernet cabled to the LAN). SoftAP **`DSC-Anchor`** comes up on channel 11 (custom SoftAP — ESPHome cannot combine `wifi:` with `ethernet:`). SoftAP-home membership is **paused** until SoftAP IP path is proven (`docs/qa/SOFTAP-FLEET-HOME.md`) — keep hub/Control/pots on Nest/home for day-to-day. Paste Anchor BSSID into hub `bridge_mac` / Lock only after SoftAP join + HA SoftAP route work. Automatic SoftAP hello to hub `DSC-Setup-*` for the bridge is deferred (F-014).
9. On the phone portal, confirm Control/pots appear under **Devices**, then tap **Finish setup**. Do **not** SoftAP-primary Hub/Control/Sonoffs until the SoftAP IP gate passes.
