...
**Live train (in tree):** HA surface **6.2.0** · hub / Control / pots / bridge / Sonoffs **5.2.0** · Sync **5.1.4+**  
(Fleet chip compares firmware major.minor only — surface **6.x** beside firmware **5.2.x** stays `ok`.)
...
- **React panel** (`/dsc-hub`, surface **6.2.0**) — Ops · Plant · Advanced · System;
  Plant Seat + pot→tent placement on The Dash. Lovelace `dsc-hub-pro` is the hidden fallback.
- **Build a Plant** (Plant → Compose) — strain · soil % · nutrients · light · climate Want → roster / pot.
...
| [`docs/qa/LIVE-UI-CUSTOM-PANEL.md`](docs/qa/LIVE-UI-CUSTOM-PANEL.md) | React `/dsc-hub` surface **6.2.0** · Plant Seat · pot→tent |
...
| Piece | Path |
|---|---|
| React panel | `homeassistant/custom_components/dsc_hub/` → sidebar **`/dsc-hub`** (surface **6.2.0**) |
| Sync add-on | [`dsc-hub-sync/`](dsc-hub-sync/) (**5.1.4+** ships Build a Plant + catalog + panel www) |
| Lovelace (Pro fallback) | `homeassistant/dashboards/dsc-hub-v4-dashboard.yaml` → URL **`dsc-hub-pro`** (sidebar hidden) |
| Lovelace (Build a Plant) | `homeassistant/dashboards/dsc-build-plant-dashboard.yaml` → URL **`dsc-build-plant`** |
| Packages | `homeassistant/packages/dsc_v4_*.yaml` (incl. `dsc_v4_pot_tent.yaml`) |
| Config snippet | `homeassistant/configuration.snippet.yaml` (`dsc_hub:` + packages + YAML fallbacks) |
