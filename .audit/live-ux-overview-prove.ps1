# Live UX Pass 3 — Overview Pi prove: SPA hotpatch + HTTP health/rooms/journals/fleet
...
$SpaTar = Join-Path $env:TEMP "live-ux-overview-spa.tgz"
$RemoteSh = Join-Path $Repo ".audit\live-ux-overview-prove.sh"
$SpaDist = Join-Path $Repo "frontend\spa-dist"
$LocalEvid = Join-Path $Repo ".audit\live-ux-overview-prove-evidence.json"
...
if ($localJs -notmatch "index-C8GkS5XE\.js") {
  Write-Warning "Expected Task 8/9 bundle index-C8GkS5XE.js; found $localJs"
}
...
& pscp @pscpArgs $SpaTar "${plinkTarget}:/tmp/live-ux-overview-spa.tgz"
if ($LASTEXITCODE -ne 0) { throw "pscp spa failed" }
& pscp @pscpArgs $RemoteSh "${plinkTarget}:/tmp/live-ux-overview-prove.sh"
if ($LASTEXITCODE -ne 0) { throw "pscp script failed" }

Write-Host "Run remote Overview prove on Pi..."
$remoteCmd = "tr -d '\r' < /tmp/live-ux-overview-prove.sh > /tmp/live-ux-overview-prove.run.sh; bash /tmp/live-ux-overview-prove.run.sh"
& plink @("-batch", "-hostkey", $HostKey, "-pw", $PiPassword, $plinkTarget, $remoteCmd)
if ($LASTEXITCODE -ne 0) { throw "remote Overview prove failed exit=$LASTEXITCODE" }

Write-Host "Pull evidence JSON..."
& pscp @pscpArgs "${plinkTarget}:/tmp/live-ux-overview-prove-evidence.json" $LocalEvid
