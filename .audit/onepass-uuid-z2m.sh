...
for path in /compose /roster /settings/compose; do
  echo "=== $path ==="
  curl -s "http://127.0.0.1:8787$path" -o /tmp/ep.json -w "http=%{http_code} bytes=%{size_download}\n"
  python3 - <<'PY'
import json
d=json.load(open("/tmp/ep.json"))
print(type(d).__name__, list(d)[:20] if isinstance(d,dict) else f"list[{len(d)}]")
found=[]
def walk(o, path=""):
...
# z2m db device count
echo "=== z2m data ==="
echo Digital | sudo -S ls -la /var/lib/dsc-hub/z2m/ 2>/dev/null | head -20
echo Digital | sudo -S docker exec dsc-hub-z2m ls -la /app/data 2>/dev/null | head -20
# mosquitto bridge devices
