...
**Unresolved promote (2026-08-10, `05f189a` + harden `096acb4` + guards `c1d6881`):**
- After SF end-link: promote high-frequency `lineage_unresolved` literals → exact `strain_canonical` + rewrite `parent_of` edges.
- Promoter rejects UI/marketing garbage (length, word count, review-chrome / family-tree phrases) plus newlines, leading punctuation, pure digits, and photoperiod chrome. See **Unresolved promote ops** below.
...
## Unresolved promote ops (`05f189a` / `096acb4` / `c1d6881`)
...
flowchart TD
  ...
  gates -->|fail| skip
  gates -->|pass| shape["newline / leading punct / digit / photoperiod"]
  shape -->|fail| skip
  shape -->|pass| upsert["upsert_canonical + rewrite parent_of"]
...
| Newline / leading punct (`c1d6881`) | `"\n" in literal` **or** starts with `>` / `(` | Scraped list/blockquote chrome |
| Pure digits (`c1d6881`) | `name_norm.isdigit()` | Not a strain identity |
| Photoperiod chrome (`c1d6881`) | `feminized photoperiod` / `landrace influence` (case-insensitive) in literal | Marketing descriptors, not parents |
...
- **Shape gates are reject-only.** Newlines, leading `>`/`(`, pure digits, and photoperiod chrome increment `skipped_junk` — they do not create canonicals.
