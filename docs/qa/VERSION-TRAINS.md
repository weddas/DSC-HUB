...
`dsc-hub-sync/config.yaml` (tip `335ddc9` Dashboard 7.0).
...
| HA product surface | **7.0.0** | `sensor.dsc_ha_surface_version` ← `packages/dsc_v4_version.yaml` |
| Integration bookkeeping | **7.0.0** | `SURFACE_VERSION` in `const.py` → `hass.data["dsc_hub"]["surface_version"]` only |
...
1. **`dsc_expected_release` = firmware train only** (`5.2.0`). Never set it to the React surface (`7.0.0`).
...
| `sensor.dsc_ha_surface_version` | **7.0.0** |
...
| Panel chrome | `SURFACE 7.0.0` · primary tabs Live/Grow/Tune/Fleet |

## Upgrade path (5.1.x / 6.x surface site → live train)
...
3. Confirm surface **7.0.0** and expected release **5.2.0** (edit helper if still on an old initial).
...
7. If Twin/Build Lit cards changed: wait for `chore(hacs): sync dist/…` (or run `./scripts/sync-hacs-dist.sh`) then HACS Redownload.
8. Smoke Pass 7.0 checklist in [`LIVE-UI-CUSTOM-PANEL.md`](LIVE-UI-CUSTOM-PANEL.md) — legacy `#/ops/home` redirects to Mission.

## Pitfalls

- Root README / older RELEASE text that say HA surface **5.2.0** or **6.x** are stale — surface is **7.0.0**.
- Putting **7.0.0** into `dsc_expected_release` makes every firmware device look off-train.
- Assuming Sync OTA’d devices — it only stages stubs/packages.
- Conflating `manifest.json` `0.1.0` (integration package) with product surface **7.0.0**.
...
- [`LIVE-UI-CUSTOM-PANEL.md`](LIVE-UI-CUSTOM-PANEL.md) — surface 7.0 guided shell + lockstep
