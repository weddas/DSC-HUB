...
Collate scripts/quarantine_junk_literals.py
        ...
        scripts/promote_unresolved_literals.py   # exact; UI/marketing gates
...
**Unresolved promote (post end-link):** `quarantine_junk_literals.py` then `promote_unresolved_literals.py` — exact-only; reject long/UI/review-chrome parents (`096acb4`). Ops: contract § Unresolved promote ops. Workset after promote: `lineage_unresolved`≈8.7k (honest leftovers).
