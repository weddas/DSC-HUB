...
flowchart LR
  ...
  capture["operator CF pass<br/>_pw_strain_db_capture"] --> sdb["_pw_scrape_strain_database<br/>headed Chrome 8-20s"]
...
### StrainDB CF soft retries (`S-STRAINDB`)
...
flowchart TD
  ...
  rewarm -->|still blocked| stop["exit CF — operator capture"]
  cf -->|no| parse["parse + stage"]
```

**Constraints / pitfalls (verified against `_pw_scrape_strain_database.py`):**
...
- Resume only after a fresh operator unlock; keep 8–20s delay; checkpoint often.
