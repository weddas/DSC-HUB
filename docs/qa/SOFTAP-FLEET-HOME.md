...
**Trigger commits:** `c47f5f1` (SoftAP-primary pots + hub↔bridge ESP-NOW) · `4a4160c` (F-004 SoftAP orphan alerts retarget) · `31fe424` (SoftAP-local **6.0.0.0** — Nest STA removed from lab wifi).  
**Builds on:** SoftAP-primary restore `21e34e5` (BSSID pin + max STA 10) and SoftAP L3 prove (same day).  
**Supersedes:** Nest-first pause `5893ea6` / docs that say “pots stay Nest-only”; SoftAP STA broadcast as the bridge path; F-004 “Nest channel lock” as the HA heal cue; dual Nest+SoftAP lab membership after SoftAP-local.  
...
**Live ops log:** [`docs/FOLLOWUPS.md`](../FOLLOWUPS.md) SoftAP fleet home § · SoftAP-local 6.0.0.0 § · F-004 SoftAP retarget §

## Intent

Grow devices join SoftAP `DSC-Anchor` on the WT32-ETH01 so they **stop flapping across home APs** (that made ESP-NOW unusable). Bridge reaches HA on Ethernet and drives Sonoffs on `192.168.4.0/24`.

**SoftAP-local (`31fe424` / train 6.0.0.0):** lab wifi YAML is SoftAP-only (Anchor BSSID pin + static SoftAP map). Nest STA entries are **gone** from hub/Control/pot/Sonoff lab wifi. Device **Fallback AP** / USB is the brick recovery path — do **not** SoftAP-hold to bounce onto Nest after SoftAP-local is flashed.
...
## SoftAP-local 6.0.0.0 cutover (`31fe424`)

| Field | Value |
|---|---|
| Membership | SoftAP `DSC-Anchor` only (no Nest STA in lab wifi) |
| Static map | Hub `.10` · Control `.11` · Pots `.12`–`.15` · Sonoffs `.20`–`.23` · Bridge eth `.66` |
| HA hosts | ESPHome entries sticky SoftAP map (wrote with core stopped so Nest hosts could not overwrite) |
| SoftAP NAPT OTA | **Broken / unproven** — mid-upload fail on SoftAP path; Nest-hold OTA workaround retired after SoftAP-local |
| Brick recovery | Device Fallback AP / USB serial — not Nest OTA |
| Open | Pot3 dead (F-003); SoftAP NAPT OTA; Pot4 soak / glass Fleet Fix residual |

```mermaid
flowchart TB
  subgraph softap ["SoftAP-local 6.0.0.0"]
    devices["Hub/Control/Pots/Sonoffs SoftAP STAs"]
    bridge["ETH01 SoftAP+NAPT .66"]
  end
  nest["Nest / home Wi-Fi"] -.->|"removed from lab wifi"| devices
  devices --> bridge
  bridge --> ha["HA via SoftAP L3"]
  otaFail["SoftAP NAPT OTA"] -.->|"unproven"| devices
  usb["Fallback AP / USB"] -->|"brick recovery"| devices
```

## Current membership (SoftAP-local after `c47f5f1` + `31fe424`)
...
flowchart LR
  ...
  nest["Nest / home Wi‑Fi"] -.->|"removed SoftAP-local"| hub
  nest -.->|"removed SoftAP-local"| control
  nest -.->|"removed SoftAP-local"| pots
  nest -.->|"removed SoftAP-local"| sonoffs
```

## Wifi YAML rules (verified)

- SoftAP-local lab wifi: SoftAP SSID only (no Nest STA network list entry).
- SoftAP entry pins Anchor BSSID **`58:2A:BD:60:3C:1D`** via `wifi_bssid` / `softap_bssid`.
- **Never** set SoftAP `bssid: 00:00:00:00:00:00` — the pin never matches → preferred nets fail → Fallback Hotspot.
...
- Lab stubs may still pass unused Nest `wifi_ssid` / `wifi_password` substitutions — ignore until stubs next cleaned.
...
SoftAP L3 **proved** 2026-08-10 (LAN/HA ping `.4.1` + hub `.4.10`). If HA cannot reach `.4.x`, use USB Install / device Fallback AP — do **not** reintroduce Nest STA as the steady state after SoftAP-local. SoftAP NAPT OTA remains unproven (FOLLOWUPS SoftAP OTA §).
...
| Nest-first / Nest STA returns after SoftAP-local | SoftAP demoted or Nest network re-added — restore SoftAP-local lab wifi |
| SoftAP NAPT OTA treated as healthy | Mid-upload fails — use USB / Fallback AP until SoftAP OTA proven |
...
| SoftAP-primary dual Nest+SoftAP STA | Nest as emergency fallback network | Superseded by SoftAP-local **6.0.0.0** (`31fe424`) — Nest STA removed |
| SoftAP-hold Nest bounce for OTA | Temporary SSID change to force Nest | **Do not** after SoftAP-local — no Nest STA left |
