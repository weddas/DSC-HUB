...
**Status:** Scaffold **landed** (still on tip `c1eaedd`) — `AirflowParticleViz` + lazy `AirflowParticleScene` on Climate “Air path”. SVG `AirPathMap` is the live lung (unused `LungLoop` removed). Trim panel / full particle field / CFD hints still incomplete (Phase D2).  
