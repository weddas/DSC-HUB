...
**Tip commit:** `47f6622` · **Firmware:** `7.0.0.0` · **Brain/SPA:** `7.0.0-dev` until island soak + tag `v7.0.0`.

Prior tips still apply: Pi appliance ship (`c4eb97f`), AP PSK alignment (`8867b33`), Native API **noise_psk** + WiFi-pref clear + env recreate (`db85cbc`). This tip migrates the Pi SPA to **FleetSnapshot** reads, adds brain **`POST /control/service`** and **`GET /history`**, and ships SPA via **`Dockerfile.prebuilt`** with **eth0 bring-up** so Pi image builds can reach Docker Hub.
...
flowchart TD
  ...
  ws --> provider
  provider --> pages[Live_Climate_Tune_Fleet]
...
| `frontend/src/lib/fleetFromHass.ts` | HA lab adapter only |
...
`POST /control/service` body: `{ "domain", "service", "data": { "entity_id", … } }`.

| Domain | Allowed | Effect |
|---|---|---|
| `input_boolean` | mapped in-service entities | `upsert_inventory(…, in_service=)` |
| `switch` | hub demand/mode switches + Sonoff `main_relay` entities | Native API `switch_command` |
| `number` / `input_number` | hub targets (temp/RH/VPD/clone bands) | Native API `number_command` |
...
| `/control/service` 400 | entity not in allow-lists | Only mapped hub/Sonoff/in-service entities (see **Control proxy**) |
