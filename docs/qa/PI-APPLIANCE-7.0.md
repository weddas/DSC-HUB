...
**Tip commit:** `f10ad40` · **Firmware:** `7.0.0.0` · **Brain/SPA:** `7.0.0` (tag `v7.0.0` after remaining fleet OTA + soak sign-off).

Prior tips still apply through ingest recovery (`53ecec7`). This tip train ships the **Pi island product** (`f4299f6`): Phase E splits native **`GET /fleet`** from **`GET /fleet/computed`**, surface **7.0.0**, ESPHome **2025.12.4** firmware YAML fixes, **fleet flash / soak / island-proof** operator scripts, POT3 gated OOS. Follow-ups (`504b82d`, `f10ad40`): drop pot Modbus `turnaround_time` (needs ESPHome 2026.8+); quote flash seat list on Windows; Sonoff **text** `Firmware Version` → `7.0.0.0` (project.version alone was not enough for the fleet chip). SPA bundle `index-BJFHGKyp`. Live evidence: [`docs/AUDIT-2026-08-26.md`](../AUDIT-2026-08-26.md) · Soak log: [`docs/ops/SOAK-2026-08-26.md`](../ops/SOAK-2026-08-26.md).
...
| Train | SoT | Notes |
|---|---|---|
| **Pi product** | FW **7.0.0.0**, brain/SPA **7.0.0** | `*-wifi-pi.yaml`, `10.42.0.0/24` |
...
**Residual (tip `53ecec7`):** ingest/control maps prefer ESPHome 2026 slug `grow_mat_demand`, but `appliance_driver.DEMAND_TO_SEAT` still keys only legacy `growmat_demand`. Heatmat relay may stay idle on Pi until the driver accepts both OIDs (dehumidifier/heater/humidifier seats already match live slugs).
...
| GET | `/fleet` | **Native FleetSnapshot** + inventory (no `hass_extras` on the hot path). Compat: `?include_computed=true` / `?include_hass=true` still embeds helpers |
| GET | `/fleet/computed` | **Phase E** dash helpers only — `{ hass_extras: … }` (CFM, alerts, fleet chip). SPA polls this separately from `/fleet` |
| WS | `/ws/fleet` | Native FleetSnapshot + inventory ~2 s (not computed helpers) |
| POST | `/control/service` | HA-shaped `{domain, service, data}` → Native API / inventory / helpers |
| POST | `/control/demand` | Convenience `{seat, on}` → hub demand switch (`heater` / `heatmat` / `humidifier` / `dehumidifier` / `ac` / `clone_humidifier`) |
| GET | `/history` | `entity_id` + `hours` (0.25–168) → `{t,v}` points from `fleet_history` |
| GET | `/grow-log` | Demand / select transition log (`hours`, `limit`) |
...
flowchart TD
  ...
  fs --> get["GET /fleet"]
  fs --> ws["WS /ws/fleet"]
  fs --> computed["GET /fleet/computed"]
  get --> provider[FleetProvider_source_pi]
  ws --> provider
  computed --> brain[BrainProvider_shim]
...
**Phase E:** Live vitals stay on `/fleet` + `/ws/fleet`. Dash helpers (`hass_extras`) live on `/fleet/computed`. Pi `useBrain` calls `get_fleet_state()` + `get_fleet_computed()`; `fleetToHass(fleet, computed)` is a thin shim until pages drop HA-shaped reads.
...
| `brain/dsc_brain/api.py` | `/fleet` native; `/fleet/computed` helpers; optional `?include_computed` |
...
| `frontend/src/lib/fleetApi.ts` | `get_fleet_state`, `get_fleet_computed`, `call_service`, `post_demand`, history/grow-log |
| `frontend/src/hooks/useFleetActions.ts` | Pi → `/control/service` (+ demand helper); HA → `hass.callService` |
| `frontend/src/hooks/useHistory.ts` | Pi → `/history`; HA → history WS |
| `frontend/src/pages/DashHomePage.tsx` | `#/ops/home` — HA Home layout parity (bands + grow log + kit) |
| `frontend/src/components/BandChartHost.tsx` | Shared band chart provider (tip `53ecec7`) |
| `frontend/src/components/DashHomeSections.tsx` | Band gauges + 24h sparklines, ESP link chips, grow log panel |
| `frontend/src/viz/gaugeTheme.ts` | Gauge styling for Dash Home |
| `frontend/src/hooks/useBrain.tsx` | Polls `/fleet` + `/fleet/computed`; merges `computed.hass_extras` into Pi Hass shim |
| `frontend/src/main.tsx` | Pi boot: `BrainProvider` → `FleetProvider source="pi"` |

**Constraint:** Default `GET /fleet` does **not** include `hass_states` (tests assert this). Pass `?include_hass=true` only for legacy consumers. Tip `bfd6495` always attaches **`hass_extras`** (synthetic CFM / roster / runtime / dash entities) — separate from `hass_states`.

### Hub sensors + binaries (tips `4aa67c5` → `bfd6495`)

**Intent:** Mission / Climate / Root / Live / **Dash Home** need room + clone tent vitals, photoperiod binaries, and stable VPD — without number-entity OIDs stomping climate sensors.

On each hub Native API poll (`esphome_client._fetch_device`):

| Map / step (`hub_controls.py` / `climate_math.py`) | Destination | Notes |
|---|---|---|
| `HUB_SENSOR_OID_TO_KEY` | `hub.values.<metric>` | **Exact** `object_id` only (no suffix fallthrough). Accepts id + name-slug variants: `temp_sensor`/`tent_temperature`→`temp_c`, `humidity_sensor`/`tent_humidity`→`rh_pct`, `vpd_sensor`/`vpd_kpa`/`vpd__kpa_`, `room_*`, `clone_*`, heartbeat/uptime, **fire countdowns** + **cooldown remaining** |
| `HUB_TEXT_SENSOR_OID_TO_KEY` | `hub.values.firmware_version` / `rf_status` | Text sensors (not floats). Firmware chip must use `firmware_version`, **not** ESPHome framework version |
| `finalize_hub_climate` | overwrite `vpd_kpa` / `clone_vpd_kpa` | Magnus VPD from live T/RH — hub template VPD can be stomped by number-entity ingest |
| `HUB_BINARY_OID_TO_ENTITY` | `hub.values.binaries[entity_id]` | Windows + light-catchup + root-zone fault + safety (`emergency_failsafe`, `climate_sensor_fault`, …) + pot ESP-NOW links (id + slug + `esp-now` variants) |
| `_finalize_hub_binaries` | fill missing pot ESP / root fault | If hub template did not publish, derive pot links from pot seat freshness (&lt;150s) and root fault from soil-temp plausibility |

### Hub ingest silent drops (tip `53ecec7`)

**Symptom (pre-fix):** Dash fans stuck at 0%, ladder chips idle/`0`, Full Auto / photoperiod looking “Standby”, firmware chip wrong — while hub was actually running.

**Cause:** ESPHome 2026+ publishes **name-slug** object_ids (`4_inch_intake_fan__main_`, `grow_mat_demand`, `humidifier_fire_countdown`, `sf1000_dimmer`, `tent_full_auto_mode`). Brain maps only had legacy C++ ids (`fan_intake_main`, `growmat_demand`, `sens_hum_countdown`, …). Unmapped OIDs were **silently dropped**.

**Fix:** Dual slug + legacy keys in `hub_controls.py`; control `ENTITY_TO_OID` prefers the live slug; `_hub_text_sensors_from_states` for firmware/RF; cooldown keys in `dash_computed.py`.

```mermaid
flowchart TD
  hub[Hub_ESPHome_2026] -->|object_id_slugs| poll[esphome_client]
  maps[hub_controls_dual_maps] --> poll
  poll -->|fans_countdowns_modes| controls["hub.values.controls"]
  poll -->|fire_cooldown_climate| values["hub.values"]
  poll -->|firmware_rf| text["hub.values.firmware_version"]
  values --> fleet["GET /fleet"]
  controls --> fleet
  text --> fleet
  fleet --> spa[Dash_Home_FleetState]
  audit[audit_hub_ingest.py] -->|critical_only| maps
  ci[brain-ci.yml] --> test[test_hub_ingest_critical_oids_mapped]
```

| Guard | How |
|---|---|
| Unit | `test_hub_ingest_critical_oids_mapped` — demand, fans, countdowns, Full Auto, SF1000, firmware, failsafe |
| CI | `.github/workflows/brain-ci.yml` runs critical test + full `brain/tests` on brain path changes |
| Live | `python -m scripts.audit_hub_ingest --critical-only` inside `dsc-hub-brain` (or `brain/scripts/audit_hub_ingest.py`) |

**Operator verify after deploy:**

```bash
curl -s http://10.42.0.1:8787/fleet | jq '{
  fw: .hub.firmware,
  fans: .hub.values.controls | with_entries(select(.key|startswith("fan."))),
  full_auto: .hub.values.controls["switch.dsc_hub_tent_full_auto_mode"],
  dehum_cd: .hub.values.dehumidifier_fire_countdown,
  fw_text: .hub.values.firmware_version
}'
# Fans should be non-zero when hub is blowing; countdown keys present (0.0 = idle, missing = bug)
```

Evidence + remaining non-critical unmapped (~50 WiFi/ladder-wait/in-service): [`docs/AUDIT-2026-08-26.md`](../AUDIT-2026-08-26.md).
...
flowchart LR
  ...
  poll --> vpd[finalize_hub_climate]
  vpd --> vals
  poll --> binaries["hub.values.binaries"]
  binaries --> fill[_finalize_hub_binaries]
  ...
  map --> bus
  brain --> bus
  bus --> pages[Mission_Climate_Root_DashHome]
```
...
| `hass_extras` (`computed_ops` + `dash_computed`) | CFM / fan %, roster summary, appliance runtime-today, efficacy, reduced-kit banners, coldest root, cannalib snapshot, dark-period flags |
| `history_ops.ENTITY_METRIC_MAP` | Charts + Dash band sparklines for room/clone temperature / humidity / VPD entity ids |

**Verify:**

```bash
curl -s http://10.42.0.1:8787/fleet | jq '{room: .hub.values.room_temp_c, clone: .hub.values.clone_temp_c, vpd: .hub.values.vpd_kpa, bins: .hub.values.binaries, extras: (.hass_extras|keys|length)}'
curl -s 'http://10.42.0.1:8787/history?entity_id=sensor.dsc_hub_clone_temperature&hours=6'
curl -s 'http://10.42.0.1:8787/grow-log?hours=24&limit=20'
```

### Dash Home SPA (tip `bfd6495`)

**Intent:** Pi `#/ops/home` rebuilds Lovelace **view_home** section jobs (now strip, bands, fans, ESP links, grow log, root/tank, CannaLib tiles) without requiring HA.

| Piece | Role |
|---|---|
| `DashHomePage.tsx` | Route `#/ops/home` / Live → Dash tab; `useEntityBus` + `useFleet` |
| `DashHomeSections.tsx` | `DashBandsGrid` (ArcGauge + 24h `Sparkline` via `/history`), `DashGrowLog` (`GET /grow-log`), ESP/fan/running chips |
| `ensureLocalCards.ts` | Registers local Lit cards (`dsc-the-dash-card`, system/airflow maps) from spa-dist |
| Brain `/vendor` mount | Serves `vendor/dsc-dash-fx.js` + `three.min.js` for cinematic hosts |

Lab HA climate markdown indent fix in `view_climate.yaml` is unrelated to Pi path (YAML parse hygiene only).

### `useEntityBus` (ops pages) vs `useFleetEntity` (controls)

| Hook | Job | Used by |
|---|---|---|
| **`useEntityBus`** | Drop-in for `useHass` on Pi: prefers `hub.values.controls`, then live metrics via `entityFleetMap`, else BrainProvider’s `fleetToHassCompat` + **`hass_extras`** shim | Mission, Climate, Root, Live, Grow, Light, Tune/Fleet, **Dash Home** + Honesty / DutyStrip / HubLinkLine / PlantExtra / EntityInspector / series / held-reading hooks |
...
### Native Compose / Learning helpers (tip `bfd6495` — supersedes prior HA-coupled residual)

**Intent:** Tip `4aa67c5` finished **ops** pages on FleetSnapshot but left Compose/Learning writing HA helpers → 400. Tip `bfd6495` adds Pi-native helper store + script handlers so Build-a-Plant / cal work without HA.

```mermaid
flowchart TD
  subgraph ready [Pi_ready_after_bfd6495]
    Ops[Mission_Climate_Root_Live_DashHome]
    Comp[ComposePlant_CoupledMix]
    Learn[LearningWizard]
    Ops --> bus[useEntityBus]
    Ops --> ufe[useFleetEntity]
    Comp --> hass[useHass_shim]
    Learn --> hass
    bus --> fleet[FleetSnapshot]
    ufe --> fleet
    hass --> extras[hass_extras]
    hass --> proxy["POST /control/service"]
  end
  proxy --> helpers[compose_store]
  proxy --> scripts[compose_ops_handle_script]
  proxy --> hub[hub_Sonoff_maps]
  fleet --> extras
```

| Surface | Files | What works on Pi today | Residual gaps |
|---|---|---|---|
| **Catalog search** | `lib/catalog.ts`, `CatalogResearch.tsx` | `VITE_DSC_PI=1` → `GET /v1/catalogs/{kind}` | — |
| **Compose / Build-a-Plant** | `ComposePlant.tsx`, `CoupledMix.tsx`, `compose_ops.py`, `compose_store.py` | Helpers (`input_text` / `input_select` / `input_number` / `input_datetime`) + scripts (`script.dsc_build_plant_*`, `dsc_accept_mix`, `dsc_apply_climate_want`, `dsc_cal_*`) via `/control/service` | Prefer eventual brain `/roster`/`/want` UX; do **not** invent height/chem/PPFD/NPK |
| **Learning / cal** | `LearningWizard.tsx`, `compose_ops.handle_script` | Cal start/hold/save/skip/abort/finish + curve resets | CFM curve sensors still derived from helpers + fan % |
| **Vessel / Tank** | Vessel/Tank + `hass_extras` | Vessel selects + helper-backed reads | Zigbee water-tester entities only when Z2M present |
| **Grow log** | `event_log.py`, `GET /grow-log`, `DashGrowLog` | Demand / select / fault events + boot stage line | Empty until first ingest cycle |
| **Charts / sparklines** | `useHistory.ts`, `DashBandsGrid` | Pi → `GET /history` | Unmapped entity ids still empty |

**Control proxy** (`control_ops.call_service_proxy`) — Pi domains after `bfd6495`:

| Domain | Behavior |
|---|---|
| `input_boolean` | In-service seats → inventory; other `dsc_*` → `compose_store` helpers |
| `switch` | Hub demand / mode + Sonoff `main_relay` |
| `number` / `input_number` | Hub targets in map → Native API; else helper store |
| `fan` / `light` / `select` | Hub maps → Native API; unmapped `select` → helpers |
| `input_select` / `input_text` / `text` / `input_datetime` / `datetime` | Helper store |
| `script` + `turn_on` | `compose_ops.handle_script` (build / assign / mix / climate want / cal) |
...
**Convenience:** `POST /control/demand` with `{ "seat": "dehumidifier", "on": true }` maps seat → demand entity and calls the same switch proxy (SPA `fleetApi.post_demand` / `useFleetActions`).
...
| `services/dsc-hub/pi/deploy-brain-remote.sh` | eth0 up → `compose build --pull brain` → `up -d --force-recreate`; else hot-patch; logs **SPA hash** + fleet acceptance `jq` |
| `services/dsc-hub/pi/verify-brain.sh` / `.ps1` | Post-deploy: `/health`, `/fleet` summary, appliance DEMAND keys, SPA hash in container |
| `services/dsc-hub/pi/flash-fleet-700.ps1` | Windows → pack `firmware/v4` + run OTA train on Pi ESPHome container |
| `services/dsc-hub/pi/flash-fleet-remote.sh` | Pi-side OTA loop (seat → YAML → `10.42.0.x`) |
| `services/dsc-hub/pi/island-proof.sh` / `.ps1` | Automated island checks (hub online, FW prefix, ingest audit, SPA hash) |
| `services/dsc-hub/pi/soak-check.sh` / `.ps1` | Hourly soak snapshot (fans / demands / relays / alerts; `fans_all_zero` warn) |
...
| `brain/dsc_brain/api.py` | Hashed `/assets` + `/vendor` → `Cache-Control: immutable`; `index.html` → `no-store` (avoids stale SPA hash) |
...
flowchart TD
  ...
  hotpatch --> health
  health --> verify[verify_brain_fleet_SPA_hash]
```

**Deploy modes (logged):** `image-build` when compose build succeeds; `hot-patch` when offline / Hub pull fails (Python+static copied into running container after force-recreate). Prefer **image-build** after ingest/map changes — force-recreate alone can leave a stale image.
...
curl -s http://10.42.0.1:8787/fleet | jq '.hub.online, .hub.firmware, .surface, (.inventory|length)'
curl -s http://10.42.0.1:8787/fleet/computed | jq '.hass_extras | keys | length'
# Deploy log must show: deploy mode: image-build (or hot-patch if offline)
# SPA tip: assets/index-BJFHGKyp.js — hard-refresh if browser still has old hash
```

**Pitfalls:** AP-only Pi often cannot pull `python:3.12-slim` → expect `hot-patch`. Broken IPv6 Docker DNS on island → `bring-up-eth0.sh` sets explicit IPv4 DNS. Windows `.env` CRLF → deploy still normalizes LF and deletes `.env\r` before recreate. `brain/static/` is a **deploy artifact** (gitignored) — never treat it as source of truth.

---

## Fleet flash 7.0.0.0 (ESPHome on Pi)

**Intent:** OTA the island fleet from the Pi ESPHome dashboard container (`dsc-hub-esphome`), not from a Nest/HA Builder path. Operator scripts pack `firmware/v4`, upload to the Pi, and run seats in order.

```mermaid
flowchart LR
  win[flash_fleet_700.ps1] -->|pscp_tarball| pi[Pi_tmp]
  pi --> sh[flash_fleet_remote.sh]
  sh --> esphome[dsc_hub_esphome]
  esphome -->|OTA| hub[.10]
  esphome -->|OTA_canary| pot2[.22]
  esphome -->|OTA| rest[pots_sonoffs_panel]
  rest --> fleet["GET /fleet firmware"]
```

| Seat order (default) | Host | Stub YAML |
|---|---|---|
| hub → pot2 → pot1 → pot3 → pot4 → heater → heatmat → humidifier → dehumidifier → control | `.10` … `.55` / `.11` | `dsc-hub.yaml` … `dsc-control.yaml` (Pi packages via `*-wifi-pi.yaml`) |

```powershell
# Full train (hub alone can take 10+ minutes)
services/dsc-hub/pi/flash-fleet-700.ps1

# Resume a single seat after a FAIL
services/dsc-hub/pi/flash-fleet-700.ps1 -Seats "heater"
...
On Pi (or after upload):

```bash
bash /tmp/flash-fleet-remote.sh "$PI_SUDO_PASS" 'hub pot2 pot1 pot3 pot4 heater heatmat humidifier dehumidifier control'
# Re-run one seat: bash /tmp/flash-fleet-remote.sh "$PI_SUDO_PASS" heater
```

**Constraints / pitfalls (verified):**

| Issue | Why | Fix |
|---|---|---|
| Windows seat list splits into “unknown seat” | `plink` expands unquoted `$Seats` → remote sees `hub` only / leftover words | Tip `f10ad40` quotes `'$Seats'` in `flash-fleet-700.ps1` |
| Sonoff fleet chip stuck on `5.2.0` after OTA | `project.version` was `7.0.0.0` but text sensor lambda still returned `5.2.0` | Tip `f10ad40` — both must match product semver |
| Pot compile fails on Pi ESPHome **2025.12.4** | `modbus.turnaround_time` needs ESPHome **2026.8+** | Tip `504b82d` — use `send_wait_time` only on Pi train |
| POT3 moisture null / chemistry noise | Hardware F-003 | Keep `in_service: false` (settings migration); island-proof warns if still in service |
| Flash from Nest Builder | Wrong Wi-Fi / secrets / SoftAP stubs | Use Pi ESPHome `:6052` + `wifi-pi` packages |

**Firmware version surfaces:** Brain `/fleet` reads the device **text** `Firmware Version` sensor (and project metadata where present). Always bump **both** `esphome.project.version` and the template text lambda when cutting a fleet train.

---

## Island proof + 24h soak

Operator still confirms Nest + HA climate automations are off. Scripts automate the rest:

```powershell
services/dsc-hub/pi/island-proof.ps1   # health, hub online, FW prefix 7.0.0*, ingest audit, SPA hash
services/dsc-hub/pi/soak-check.ps1     # hourly JSON snapshot; watch warn: fans_all_zero
```

Log: [`docs/ops/SOAK-2026-08-26.md`](../ops/SOAK-2026-08-26.md). Tag `v7.0.0` only after soak sign-off.
...
4. Build/flash FW **7.0.0.0** (`wifi-pi` packages) with `wifi_ssid`/`wifi_password` matching the Pi AP. Prefer `services/dsc-hub/pi/flash-fleet-700.ps1` (order: **hub → Pot2 canary → pots → Sonoffs → panel**). Quote the seat list when invoking remote bash from Windows.
5. Disable HA demand-follower automations / HA ESPHome integrations for the tent (keep packages until soak).
6. Island proof: Nest + HA off; tent on Pi AP; run `island-proof.ps1`; fleet chip / health `expected_firmware` **7.0.0.0**.
7. eth0 up: Settings → Test Ollama + Test CannaLib. eth0 down: integrations HELD; local cannalib fallback if present.
8. Hourly `soak-check.ps1` for 24h ([`SOAK-2026-08-26.md`](../ops/SOAK-2026-08-26.md)); tag `v7.0.0` only after soak.
...
| `/control/service` 400 | entity / script not mapped | See **Control proxy** / `compose_ops.handle_script` / `hub_controls.py` |
...
| Fans always 0% / ladder chips idle while hub runs | ESPHome 2026 slug OIDs unmapped (silent drop) | Tip `53ecec7` dual maps; `audit_hub_ingest.py --critical-only`; confirm `/fleet` has fans + `*_fire_countdown` |
| Firmware chip shows ESPHome framework (e.g. `2026.8.0`) | Reading wrong version field | Expect `hub.values.firmware_version` / `.hub.firmware` product semver |
| Sonoff chip shows `5.2.0` after 7.0 OTA | Text sensor lambda not bumped with `project.version` | Tip `f10ad40` — flash `dsc-sonoff-common` with both at `7.0.0.0` |
| `flash-fleet` only flashes hub / “unknown seat” | Windows seat args unquoted across `plink` | Tip `f10ad40` — `'$Seats'` in `flash-fleet-700.ps1`; resume with `-Seats "heater"` |
| Pot YAML fails on Pi ESPHome 2025.12.4 | `turnaround_time` unsupported | Tip `504b82d` — `send_wait_time` only until ESPHome 2026.8+ |
| Stale SPA after deploy (old bundle hash) | Cached `index.html` or hot-patch-only | Prefer `image-build`; tip sets `no-store` on index; hard-refresh; `verify-brain` prints SPA hash |
| Mission / Root / Dash room or clone blank on Pi | room/clone not in `hub.values` / stale SPA | Confirm tip ingest maps; `jq '.hub.values \| {room_temp_c, clone_temp_c, vpd_kpa, binaries}'`; hard-refresh for `index-BJFHGKyp` |
| VPD absurd / stuck at number-entity value | number OID stomped sensor before tip | Tip recomputes via `finalize_hub_climate`; confirm exact OID map (no suffix fallthrough) |
| Window / ESP link binaries missing | `hub.values.binaries` empty + no pot freshness | Check OID variants; `_finalize_hub_binaries` fills pot links when hub templates quiet |
| Dash Home empty / no sparklines | stale SPA or no history samples | Hard-refresh `index-BJFHGKyp`; wait for ingest; `GET /history` for band entity |
| Heatmat Sonoff never follows grow-mat demand | `appliance_driver` only maps legacy `growmat_demand` | Code: add `grow_mat_demand` to `DEMAND_TO_SEAT` (docs residual); dehum/heater/hum OK |
| Grow log empty | no demand/select transitions yet | Boot stage line appears after first hub control poll; `GET /grow-log` |
| `hass_extras` missing CFM / roster | looking on `/fleet` hot path | Phase E: `GET /fleet/computed` (or `?include_computed=true`); reconnect SPA |
...
| Compose commit / Learning cal fail on Pi | helpers DB not initialized / unknown script | Tip handles helpers + `script.dsc_*` via `compose_store` / `compose_ops`; check script id spelling |
...
# includes critical ingest OID manifest, climate VPD finalize, pot ESP binary fill,
# grow-log API, compose commit, hass_extras, /control/demand

# Live Pi (after deploy):
# python -m scripts.audit_hub_ingest --critical-only   # inside dsc-hub-brain
# services/dsc-hub/pi/verify-brain.sh
...
# tip ships spa-dist assets/index-BJFHGKyp.js + vendor/
```

Monitoring: Uptime Kuma → `GET /health`; fleet WS → `ws://dsc-brain.local:8787/ws/fleet`; helpers → `GET /fleet/computed`. After deploy, hard-refresh SPA and confirm `#/ops/home` bands + grow log, fans/countdowns on `/fleet`, and Mission/Climate/Root show room/clone/VPD when hub online. See [`docs/AUDIT-2026-08-26.md`](../AUDIT-2026-08-26.md).
