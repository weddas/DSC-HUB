...
**Tip commit:** `bfd6495` · **Firmware:** `7.0.0.0` · **Brain/SPA:** `7.0.0-dev` until island soak + tag `v7.0.0`.

Prior tips still apply: Pi appliance ship (`c4eb97f`), AP PSK alignment (`8867b33`), Native API **noise_psk** + WiFi-pref clear + env recreate (`db85cbc`), FleetSnapshot SPA + `/control` + `/history` + **Dockerfile.prebuilt** (`47f6622`), hub **`hub.values.controls`** + Climate `useFleetEntity` (`c1a451a`), room/clone + binaries + **`useEntityBus`** (`4aa67c5`). This tip ships **Dash Home SPA** (`#/ops/home`) with HA Home layout parity, **exact-OID climate ingest** + **Pi-side VPD**, **ESP/pot link binary fallbacks**, **`hass_extras`** (CFM / roster / runtime), **native Compose/Learning helpers + scripts**, **grow log** (`GET /grow-log`), band **sparklines**, and SPA bundle `index-BezFeFI8` (+ `/vendor` for dash-fx / three).
...
| `frontend/src/lib/fleetApi.ts` | `get_fleet_state`, `call_service`, `get_entity_history`, `get_grow_log` |
...
|---|---|---|
| `HUB_SENSOR_OID_TO_KEY` | `hub.values.<metric>` | **Exact** `object_id` only (no suffix fallthrough). Accepts id + name-slug variants: `temp_sensor`/`tent_temperature`→`temp_c`, `humidity_sensor`/`tent_humidity`→`rh_pct`, `vpd_sensor`/`vpd_kpa`, `room_*`, `clone_*`, heartbeat/uptime |
...
| `HUB_BINARY_OID_TO_ENTITY` | `hub.values.binaries[entity_id]` | Windows + light-catchup + root-zone fault + pot ESP-NOW links (id + slug variants) |
...
| Mission / Root / Dash room or clone blank on Pi | room/clone not in `hub.values` / stale SPA | Confirm tip ingest maps; `jq '.hub.values \| {room_temp_c, clone_temp_c, vpd_kpa, binaries}'`; hard-refresh for `index-BezFeFI8` |
...
| Dash Home empty / no sparklines | stale SPA or no history samples | Hard-refresh `index-BezFeFI8`; wait for ingest; `GET /history` for band entity |
...
# includes climate VPD finalize, pot ESP binary fill, grow-log API, compose commit, hass_extras
...
# tip ships spa-dist assets/index-BezFeFI8.js + vendor/
```

Monitoring: Uptime Kuma → `GET /health`; fleet WS → `ws://dsc-brain.local:8787/ws/fleet`. After deploy, hard-refresh SPA and confirm `#/ops/home` bands + grow log, and Mission/Climate/Root show room/clone/VPD when hub online.
