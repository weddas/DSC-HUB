...
**Tip commit:** `53ecec7` · **Firmware:** `7.0.0.0` (live hub may still report `6.0.0.0` until flash) · **Brain/SPA:** `7.0.0-dev` until island soak + tag `v7.0.0`.

Prior tips still apply: Pi appliance ship (`c4eb97f`), AP PSK alignment (`8867b33`), Native API **noise_psk** + WiFi-pref clear + env recreate (`db85cbc`), FleetSnapshot SPA + `/control` + `/history` + **Dockerfile.prebuilt** (`47f6622`), hub **`hub.values.controls`** + Climate `useFleetEntity` (`c1a451a`), room/clone + binaries + **`useEntityBus`** (`4aa67c5`), Dash Home + Pi-native compose (`bfd6495`). This tip **recovers hub ingest silent drops** (ESPHome 2026 name-slug dual maps for fans, fire countdowns, mode switches, firmware text), ships **FleetState Dash Home** polish (`BandChartHost`, gauge theme), **`POST /control/demand`**, SPA cache headers, **image-build deploy + verify gates**, CI `brain-ci.yml` + `audit_hub_ingest.py`, and SPA bundle `index-BXZwXQYP`. Live evidence: [`docs/AUDIT-2026-08-26.md`](../AUDIT-2026-08-26.md).
...
| `frontend/src/lib/fleetApi.ts` | `get_fleet_state`, `call_service`, `post_demand`, `get_entity_history`, `get_grow_log` |
...
| `frontend/src/hooks/useBrain.tsx` | Merges `fleet.hass_extras` into Pi Hass shim |
...
# SPA tip: assets/index-BXZwXQYP.js — hard-refresh if browser still has old hash
```
...
| Mission / Root / Dash room or clone blank on Pi | room/clone not in `hub.values` / stale SPA | Confirm tip ingest maps; `jq '.hub.values \| {room_temp_c, clone_temp_c, vpd_kpa, binaries}'`; hard-refresh for `index-BXZwXQYP` |
...
| Dash Home empty / no sparklines | stale SPA or no history samples | Hard-refresh `index-BXZwXQYP`; wait for ingest; `GET /history` for band entity |
...
| `hass_extras` missing CFM / roster | brain pre-tip or WS payload stale | Confirm `/fleet` has `hass_extras`; reconnect `/ws/fleet` |
...
# tip ships spa-dist assets/index-BXZwXQYP.js + vendor/
```

Monitoring: Uptime Kuma → `GET /health`; fleet WS → `ws://dsc-brain.local:8787/ws/fleet`. After deploy, hard-refresh SPA and confirm `#/ops/home` bands + grow log, fans/countdowns on `/fleet`, and Mission/Climate/Root show room/clone/VPD when hub online. See [`docs/AUDIT-2026-08-26.md`](../AUDIT-2026-08-26.md).
