...
**Tip:** `ee636f9` · spa-dist `index-BoyhWWR_.js` · Firmware `dsc-hub-v4_0.yaml` · SPA Light / Live / `lightSchedule.ts` · Design [twin-sf1000](../superpowers/specs/2026-08-29-twin-sf1000-design.md) · Pass 4 [pass4 design](../superpowers/specs/2026-09-01-live-ux-pass4-twin-integrated-design.md) · Pass 5 [pass5 design](../superpowers/specs/2026-09-01-live-ux-pass5-followup-design.md) · [pass5 gate](../../.superpowers/sdd/pass5-gate-report.md) · hotpatch [`PI-HOTPATCH.md`](PI-HOTPATCH.md)
...
## SPA / brain consumers (verified tip `ee636f9`)
...
| Phase B / C / Task 7 gate | **GREEN** (`4a40589`) — spa `index-BoyhWWR_.js` |
| Pass 5 Manual Light Hold control map | **Landed** (`d07cbd9`) — `HUB_SWITCH_ENTITY_TO_OID` + operator clear |
| Pass 5 energy confirm=false → 400 | **Canonical** — both tents |
| Pass 5 Wet→Problem + FlowSankey + CannaLib | **Proven** — program **closed** |
| Optical / physical PWM | **Not claimed** — GPIO5 reserved |

**Hardware parks (out of Live UX program):** GPIO5 optical PWM; `leak_floor_2x4` (no HW).

## Manual Light Hold (Pass 5)

| Layer | Path | Rule |
|-------|------|------|
| Control map | `hub_controls.HUB_SWITCH_ENTITY_TO_OID` | `switch.dsc_hub_manual_light_hold` → `manual_light_hold_switch` (writable) |
| Operator clear | `POST /control/service` turn_off | Only after **operator confirm**; prove scripts must not auto-clear |
| SPA | Light desk Hold chip | Gate: **MANUAL LIGHT HOLD OFF**; no “hold is on” banner |

Hold freezes schedule actuation — distinct from Twin toggle. Twin on_state may still latch shared hold in firmware; brain clear path is the operator SoT for sticky Hold after Pass 5.
...
- **GPIO5 handoff:** reserved for Twin SF1000 PWM module (operator wire-up). Durable note in FOLLOWUPS Pass 4 / Pass 5 **gate** sections — never claim wired/optical until physical confirm.
...
- Prefer **`docker stop -t 20` + `start`** for brain hotpatch (Pass 5 Task 5 `kill` hung the Pi; bare `restart` hung earlier). See [`PI-HOTPATCH.md`](PI-HOTPATCH.md).

## Ops checks

```bash
# After hub OTA — Twin entity present in brain hass/controls
# Light: Twin toggle + Got · Twin / Got · Window chip; Hold OFF after operator clear
# History: twin_sf1000_on + sf1000_on ingest; 4×8 Actual Twin; 2×4 Actual window
# Prove: .audit/live-ux-pass4-prove.ps1 PASS4_PHASE=GATE
#        .audit/live-ux-pass5-prove.ps1 PASS5_PHASE=GATE (program closed; optical N/A)
# Never paste API keys / Wi-Fi / hostkeys into walks
```

## Related

- [`../brain/LIVE-UX-HONESTY.md`](../brain/LIVE-UX-HONESTY.md) — Passes 1–5 program runbook  
- [`PI-HOTPATCH.md`](PI-HOTPATCH.md) — plink/pscp, stop+start, Hold/Twin fleet-lag flakes  
...
- [`../qa/LIVE-UX-PASS4-WALK-2026-09.md`](../qa/LIVE-UX-PASS4-WALK-2026-09.md) — Pass 4 walk  
- [`../qa/LIVE-UX-PASS5-WALK-2026-09.md`](../qa/LIVE-UX-PASS5-WALK-2026-09.md) — Pass 5 walk (**proven**) 
