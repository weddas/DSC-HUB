...
**Release:** DSC-HUB 7.0.0 — The Pi Release. Tip train: appliance (`c4eb97f`) → AP PSK (`8867b33`) → Native API / WiFi-pref (`db85cbc`) → **FleetSnapshot SPA + prebuilt deploy (`47f6622`)**.

**Full ops runbook:** [`docs/qa/PI-APPLIANCE-7.0.md`](../qa/PI-APPLIANCE-7.0.md) (architecture, LAN map, AP PSK, **noise_psk**, WiFi-pref, env recreate, FleetSnapshot `/control` `/history`, **Dockerfile.prebuilt** + eth0).  
...
- Control smoke: `POST /control/service` with a mapped number/switch
