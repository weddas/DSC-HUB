...
**Release:** DSC-HUB 7.0.0 — The Pi Release. Tip train: appliance (`c4eb97f`) → AP PSK (`8867b33`) → Native API / WiFi-pref (`db85cbc`) → FleetSnapshot SPA + prebuilt (`47f6622`) → hub.controls (`c1a451a`) → entitybus (`4aa67c5`) → Dash Home (`bfd6495`) → **ingest recovery + FleetState deploy (`53ecec7`)**.
...
7. Deploy SPA: `services/dsc-hub/pi/deploy-brain.ps1` (prebuilt Dockerfile; eth0 for image-build, else hot-patch). Run `verify-brain.ps1` / `.sh`; confirm `/fleet` fans+countdowns + SPA `index-BXZwXQYP`; hard-refresh UI.
