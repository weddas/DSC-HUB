...
**Release:** DSC-HUB 7.0.0 — The Pi Release · tip **`8b3169c`** (Root trust / station rate-dryback / stage calendar; SPA `index-CrVRU9Qb.js` live). Prior **`07bf25f`** Pass A–C + NPK producers; **`05e8471`** Climate Mode / probe rename ops closed; surface **7.3.0**, 7.4 WiP.
...
### Soil metrics + SPA (tip `8b3169c`)

Pi needs **both** when producers or Root honesty change:

1. SPA bundle with map keys `nitrogen|phosphorus|potassium`, `/fleet/computed` merge, peer MAD + station waiting copy (`index-CrVRU9Qb.js` on tip).
2. Brain container with `sensor_trust.py` (stations emit rate/dryback; stuck plant-only) and `computed_ops` / `stage_model` calendar advance.

Hot-patch SPA alone is not enough for dryback/rate or stage SoT. Runbook: [`FLEET-SOIL-METRICS.md`](FLEET-SOIL-METRICS.md) · kit SPA: [`../brain/KIT-SOT-SPA.md`](../brain/KIT-SOT-SPA.md).
