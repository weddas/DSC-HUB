...
**Tip:** SoftCal Raw + sessions API · SoftCal AI `bce7ca9` · Wave 1 assignment chrome `bae50fa` · SPA `SoftCalWizard.tsx` · `soft_cal_ai.py` · `soft_cal_history.py`
...
1. Select kit probes (1–2 on Live kit; wizard still lists SoftCal pots 1–4 for Advanced restore).
...
Table `soft_cal_sessions` (Pi SQLite via `soft_cal_history.py`):
...
| Constraint | Reality |
|------------|---------|
| `probe_n` | Required **1–4** on POST (400 otherwise) |
| `phase` | Required non-empty string |
| SPA | SoftCalWizard does **not** POST sessions yet — API is for scripts/operators; UI wiring residual |
| Ask Brain | Calls `/ai/soft-cal-advice` only — does not imply a session row was written |

Operator example (curl on Pi LAN):

```bash
curl -s -X POST http://127.0.0.1:8787/soft-cal/sessions \
  -H 'Content-Type: application/json' \
  -d '{"probe_n":1,"phase":"water","payload":{"known_ph":7.0,"n":12}}'
curl -s 'http://127.0.0.1:8787/soft-cal/sessions?probe_n=1&limit=5'
```
...
| Assignment | SoftCal OK while plant assigned (Wave 1 chip) — detach is Roster SoT |
...
- Wave 1 chrome: [`../brain/OPERATOR-WAVE1.md`](../brain/OPERATOR-WAVE1.md)
- [`LAB-WET-CAL.md`](LAB-WET-CAL.md) · [`../brain/DECISION_LOOP.md`](../brain/DECISION_LOOP.md)
