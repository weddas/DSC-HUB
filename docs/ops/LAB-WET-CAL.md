...
Peer median aligns pots relative to each other; **lab wet** stamps one channel against a known buffer solution on the **ESP**.
...
5. On Pi SPA → **Tune → Calibrate → Soil**, run **Lab wet calibration**:
   - Sets `input_number.dsc_lab_wet_pot` to the pot id
   - Runs **`script.dsc_pots_apply_lab_wet_to_esp`** (shared apply path — **not** `script.dsc_potN_lab_wet_cal`)
