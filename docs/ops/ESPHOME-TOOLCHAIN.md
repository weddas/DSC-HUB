...
| Settings UI | Settings → Device → **Firmware (ESPHome)** card: installed / latest_supported / pinned-min, build backend + disk free, **Update ESPHome**, **Roll back**, secrets / helper / disk / held-back chips, per-seat drift, **Open ESPHome Dashboard**, canary → release-the-rest rollout. |
...
1. Settings → Device → **Firmware (ESPHome)**. The card shows **installed** vs
