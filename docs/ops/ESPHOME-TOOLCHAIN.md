...
**Ownership traps (live cutover):** run the unit as `dsc`, not root. Root-owned
`.esphome` / `.device-builder*` / PlatformIO cache under the project tree breaks
the service even when the binary is correct.
