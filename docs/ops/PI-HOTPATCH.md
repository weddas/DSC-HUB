...
Verified against tip `6a89efa` (Hub **8.1.0**; aligned-card bake `89ddfa7`; kit firmware train **8.1.0.0** on ESPHome **2026.8.2**; SoftAP kit bake + real card bins + thin-catalog cannalib). Rule: [`.cursor/rules/dsc-pi-hotpatch.mdc`](../../.cursor/rules/dsc-pi-hotpatch.mdc) · AGENTS Pi hotpatch bullet. Lab host: `dsc@…`, container `dsc-hub-brain`, static `/app/static/`.
...
- Tip `2d7bfca` / published **8.1.0** product (Settings S4/S5 + Tuya T1 + SoftAP kit bake + real card bins; GitHub release live) is **not yet assumed hotpatched** to the lab Pi — verify served index hash and `/health` `version` before claiming live.
