...
Verified against tip `c3958ff`. Rule: [`.cursor/rules/dsc-pi-hotpatch.mdc`](../../.cursor/rules/dsc-pi-hotpatch.mdc) · AGENTS Pi hotpatch bullet. Lab host: `dsc@…`, container `dsc-hub-brain`, static `/app/static/`.
...
- Tip `c3958ff` product (Settings S4/S5 + Tuya T1) is **not yet assumed hotpatched** to the lab Pi — verify served index hash before claiming live.
