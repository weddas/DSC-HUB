...
> Drafted 2026-09-07 on `feat/dashboard-v2`, after the v2 dashboard passes A–H. Nothing in this document is implemented.
...
- Next: operator answers the [open questions](#open-questions-for-the-operator), then Pass S1.
...
| **H — Hub ESP** | ESPHome `number.` / `select.` / `switch.` / `time.` entities in NVS, ~70 of them (`*.dsc_hub_*`), plus the panel/probe `input_*` helpers | `/control/service` → native API; the brain re-asserts on reconnect (`hub_failover`, TTL 900 s) | write-through with the confirmed/failed flash (`EntityToggle` pattern); `DecisionLayer` when an actuator may move | hub offline → row shows `HELD` with last value and greys the control |
...
|---|---|---|---|---|
| Theme | Dark | `const` (dark only; `themes/light-mode.md` is an exploration) | Preferences › Appearance | Options: Dark · Light · Follow system. Light ships only when the token overrides in `light-mode.md` pass the contrast checks listed there; until then the row shows Dark and `Light — proposed` as a dashed option. |
...
| Setting | Default | Today | Proposed home | Notes |
|---|---|---|---|---|
| Temperature | °C | `const` (no °F anywhere) | Units | °C · °F. Display only; the brain and hub stay metric. Every `formatReading` call goes through one `useUnits` hook. |
| VPD | kPa | `const` | Units | kPa only. Row exists so the unit is stated, not chosen. |
| Conductivity | mS/cm | `const` | Units | mS/cm · µS/cm · ppm 500 · ppm 700. Root desk EC axis and CannaLib. |
| Substrate moisture | % VWC | `const` | Units | % only; stated. |
...
| Airflow | CFM | `const` | Units | CFM · m³/h. |
...
| Recipients / "two people" | — | `none` | — | Alerts › Delivery | INVENTED row, same treatment. |
...
| Setting | Default | Today | Proposed home | Notes |
|---|---|---|---|---|
| Stage rail presets (T, VPD min/max, RH min/max, hours) × 10 stages | `STAGE_RAIL` in `tentWant.ts`, mirrors `apply_stage` in hub firmware | `const` (SPA) + `F` (hub) | Climate › Stage presets | Editable table with per-cell reset; stored tier N (`stage_rail_json`) and pushed to the hub on stage change. Until the hub accepts overrides, the table is read-only with the `FIRMWARE` badge and a note that edits apply to Want bands only. |
...
| Storage | history retention (0–3650 d), stats, prune now; journal retention (new, tier N) | `UI` (System) |
...
| Developer | show entity ids on tiles, provenance mode (every derived number shows its formula), raw `/fleet` snapshot viewer, feature flags (`twin-spike`, `force3d`), show INVENTED slots, settings export / import JSON, settings change log | `none` |
...
| H | Write-through on change with the `EntityToggle` waiting → confirmed / failed treatment; `DecisionLayer` confirm when the manifest marks the entity `actuates: true`. Row greys with `HELD` when the hub is offline. |
...
| **S2 — The grow sections** | Climate / Light / Root / Sensors rows for hub helpers (tier H) through `/control/service` with the confirmed/failed treatment and `HELD` when offline; stage-rail table (read-only until the hub accepts overrides); steering targets UI; tariff and learning move to Light; per-zone offsets and clamps to Sensors; leaf offset single-sourced. | Each hub number shows the hub's echoed value within one poll; a write to a min-off time while the hub is offline greys and explains; SPA leaf VPD equals the brain's within rounding. |
| **S3 — Alerts & Automation** | Alert catalogue with enable/severity (brain KV), delivery card (toast, sound, quiet hours, snooze presets), INVENTED rows for push/recipients; Automation section with rule defaults; settings-change journal entries and the Logs filter. | Disabling an alert hides it on Alerts desk and Overview mission line; quiet hours silence sound but never the failsafe; every setting write from S1–S3 appears in Logs. |
| **S4 — System & transparency** | Time card (tz, NTP, hub/brain drift), failover TTL, brain route health, developer card (entity ids, provenance mode, raw snapshot, feature flags, export/import), reset flows with typed confirm. | Export → import on a second browser reproduces preferences; provenance mode shows a formula on every derived number. |
| **S5 — Devices restructure** | Device tab → four sub-tabs with drawers; inventory accordion by role, offline/OOS open by default (SP-P0-3); Firmware absorbs Server. | Device page fold shows ≥ 6 seats at 1280 and ≥ 3 at 390; Zigbee bind is a drawer. |

Order: S1 → S2 → S3 → S5 → S4. S5 before S4 because the Device wall is the operator's loudest open UX finding.

---

## Open questions for the operator

1. **°F at all?** The kit is metric and the brain stays metric either way. If nobody will ever use °F, Units still lists the unit but drops the switch.
2. **Light theme: ship, or keep as exploration?** `light-mode.md` says the operator surface should probably stay dark. The plan only ships Dark + Follow-system until the accent contrast fixes are done.
3. **Stage rail overrides.** Editing the presets means the hub's `apply_stage` must read them from the brain, which is firmware work. Read-only table first, or do the firmware change in S2?
4. **Hub helper writes from Settings.** Confirm that writing hysteresis, min-off times and ladder waits through `/control/service` is acceptable, given the brain re-asserts on reconnect (`hub_failover`). If the brain should own these, they become tier N with the brain pushing to the hub.
5. **Who else uses this?** People/roles/PIN are INVENTED rows until told otherwise. A read-only guest mode is cheap if wanted.
6. **Journal retention** is unbounded today; pick a default or leave it unbounded on purpose.
