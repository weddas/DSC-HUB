...
- **2026-09-07 — composed twin live**: `#/twin` composes the library into the rig (room → tents → devices → roster plants, all snapped by anchor) and binds it to Zone state via `useTwinState`; effect layers for air (duty/CFM streaks along duct paths), heat (want-band tinted volumes + shimmer), humidity (haze by RH, dew-point risk, mist) and light (brightness cones + canopy footprint); camera presets, what-if panel (flagged SIMULATED, never written), hover → entity, click → inspector. Pack-3 briefs (`docs/design/models/BRIEFS-3.md`) ask for the plant/vessel split, the 2×4 rebuilt with anchors, the real room. Evidence: `docs/FOLLOWUPS.md` § composed live scene.
- **2026-09-07 — model library built**: the operator delivered 79 three.js builder sources for both brief packs; `frontend/scripts/build-twin-models.mjs` turns them into 101 GLBs + `manifest.json` v2 (anchors as empty nodes, roles by prefix); viewer learned `emissive` / `wire` roles; `#/twin-spike?model=` browses every model. Evidence: `docs/FOLLOWUPS.md` § model pipeline.
...
- Pack 2, the generic library (48 briefs: tent family, bar/HID/T5/clip lighting, 6"/8" fans and fittings, circulation fans, vents, silencers, room-class climate, CO₂ sources, IBC/dosing/drip/flood/DWC/pots, sensors, power, mothers, drying, curing, hydro support kit): `docs/design/models/BRIEFS-2.md`. New material roles to add to `TwinViewport.toWire`: mesh_wire, glass_clear, metal_brass, rubber_black.
