...
```
Pots ──ESP-NOW + SoftAP STA──► Hub / Bridge SoftAP
Hub ──ESP-NOW + SoftAP STA──► Bridge (ETH01)
...
| Max STA | ≥14 (hub + control + 4 sonoffs + 4 pots + headroom) |
...
Bridge `dsc_api_client` hosts use SoftAP IPs for Sonoffs. HA → SoftAP clients requires a host route on HA OS: `192.168.4.0/24 via 192.168.86.66` (document + apply when validating).
...
| Pots | primary + static IP | fallback | SoftAP channel pin for ESP-NOW; low data |
...
6. Pots on SoftAP `.12`–`.15` answer HA API; ESP-NOW pot links stay healthy  
