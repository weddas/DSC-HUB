...
**Ops SoT (7.0 tip `bfd6495`):** [`docs/qa/PI-APPLIANCE-7.0.md`](qa/PI-APPLIANCE-7.0.md) — LAN map, AP PSK, **noise_psk** Native API, WiFi-pref clear, brain env recreate, **FleetSnapshot SPA**, **Dash Home**, **`hub.values.controls`** + room/clone + Pi VPD + **`hub.values.binaries`**, **`hass_extras`**, native Compose helpers/scripts, `/control` + `/history` + `/grow-log`, **`useEntityBus`**, **Dockerfile.prebuilt** + eth0 bring-up.
...
| **Pi brain** | Catalogs, Want/Need, roster, Learning, setpoint proposals, appliance driver, FleetState + hub controls/sensors/binaries, compose helpers, computed extras, grow log | Hard safety if Pi unplugged |
| **Webserver** | Presentation, advanced control, Dash Home, updates (`useEntityBus` / `useFleetEntity` client) | Catalog SoT / Want math |
...
| [`brain/dsc_brain/computed_ops.py`](../brain/dsc_brain/computed_ops.py) / [`dash_computed.py`](../brain/dsc_brain/dash_computed.py) | `hass_extras` for SPA (CFM, roster, runtime, dash flags) |
...
- **B** Brain core
- **C** Webserver MVP (FleetSnapshot SPA; tip `4aa67c5` ops-page native reads; tip `bfd6495` Dash Home + native Compose/Learning)
- **D** Close loop + Pi appliance driver (`c4eb97f` / AP PSK `8867b33` / Native API `db85cbc` / FleetSnapshot `47f6622` / hub.controls `c1a451a` / entitybus `4aa67c5` / Dash Home `bfd6495`); HA optional integration
