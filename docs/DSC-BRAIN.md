...
**Ops SoT (7.0 tip `53ecec7`):** [`docs/qa/PI-APPLIANCE-7.0.md`](qa/PI-APPLIANCE-7.0.md) — LAN map, AP PSK, **noise_psk** Native API, WiFi-pref clear, brain env recreate, **FleetSnapshot SPA**, **Dash Home**, **ESPHome 2026 dual-slug ingest** (fans / countdowns / modes / firmware), **`hub.values.controls`** + room/clone + Pi VPD + **`hub.values.binaries`**, **`hass_extras`**, native Compose helpers/scripts, `/control/service` + `/control/demand` + `/history` + `/grow-log`, **`useEntityBus`**, **Dockerfile.prebuilt** + eth0 bring-up + **verify-brain** / **brain-ci**. Live audit: [`docs/AUDIT-2026-08-26.md`](AUDIT-2026-08-26.md).
