...
**Control recovery (locked design):** restore HA-era loops on the brain, SPA client-only, hub offline = manual takeover + reconnect re-plan. See [brain/CONTROL-RECOVERY.md](brain/CONTROL-RECOVERY.md) · [design spec](superpowers/specs/2026-08-29-brain-control-recovery-design.md). Bar 1 is **not** shipped yet — Manual Takeover on the hub already exists; temporary-override reconnect does not.
