...
| N-087-PROMOTE | Unresolved literal promote (exact + UI harden) | **Done 2026-08-10:** `promote_unresolved_literals.py` (`05f189a`) + UI/marketing reject gates + quarantine phrase harden (`096acb4`) + shape guards (`c1d6881`: newline / leading punct / digits / photoperiod chrome). Ops: [`CATALOG-COLLATION-CONTRACT.md`](qa/CATALOG-COLLATION-CONTRACT.md) § Unresolved promote ops. Post-pass `lineage_unresolved`≈8.7k leftovers (no fuzzy). |
