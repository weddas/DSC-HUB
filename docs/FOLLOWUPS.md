...
| N-087-DENSIFY | Catalog densify + staging drain | Offline densify then full local staging drain (`c8e0cd4`): obs≈126k; alias≈26.7k (merch/`page N`/`post N` cleaned); height_cm≈13.4%; SF merged+linked. Projectors: `project_observations_from_raw` (Wikileaf + `info`/`more_info`/`about_info` + RQS excerpts), `harvest_bank_aliases`, `clean_merch_aliases`. Next = scrape-only thin chase. Workset `C:\DSC\collation\`; ops: [`CATALOG-COLLATION-CONTRACT.md`](qa/CATALOG-COLLATION-CONTRACT.md) § Staging drain ops · [`CATALOG-THIN-FIELDS.md`](qa/CATALOG-THIN-FIELDS.md). |
