...
Tip code unchanged at the time. Docs expanded honesty for surfaces that still imported `useHass` after ops pages moved to `useEntityBus`. **Superseded for Compose/Learning by tip `bfd6495`** (native helpers/scripts) — keep only as history of the residual that tip closed.
...
## 2026-08-24 — Dash Home + Pi-native compose/climate (`bfd6495`)

Tip: Dash Home SPA (`#/ops/home`) with band sparklines + grow log; hub climate exact-OID ingest + `finalize_hub_climate` VPD; pot ESP link binary fallbacks; `hass_extras` (`computed_ops`/`dash_computed`); `compose_store` + `compose_ops` for helpers/scripts; `GET /grow-log`; SPA `index-BezFeFI8` + `/vendor`; HA `view_climate.yaml` markdown indent fix.

### docs

- [`docs/qa/PI-APPLIANCE-7.0.md`](qa/PI-APPLIANCE-7.0.md) — tip SHA `bfd6495`; Dash Home; climate/VPD/ESP binaries; native Compose/Learning (supersedes HA-coupled residual for those surfaces)
- [`docs/brain/WEBUI.md`](brain/WEBUI.md), [`docs/DSC-BRAIN.md`](DSC-BRAIN.md), brain READMEs, CHANGELOG

### red-flag

- `_HUB_SWITCH_ENTITY_TO_OID` NameError on hub switch `/control/service` still present in tip code (fan/number/select OK).

### soak / next-plan

1. Deploy + hard-refresh: confirm SPA `index-BezFeFI8`; `#/ops/home` bands + grow log when hub online.
2. `jq '{vpd:.hub.values.vpd_kpa, clone:.hub.values.clone_vpd_kpa, bins:.hub.values.binaries, extras:(.hass_extras|keys|length)}'` on `/fleet`; `GET /grow-log`.
3. Smoke Compose commit + Learning cal on Pi (helpers/scripts); confirm no false “fleet outage” on 400.
4. Prefer this tip’s docs PR for SoT; close/supersede #89–#95 after review if overlapping.
5. Alias `_HUB_SWITCH_ENTITY_TO_OID` before soak relies on hub switch proxy.
6. Do **not** invent height/chem/PPFD/NPK; do not paste secrets into Wiki/PRs.
...
### docs (tip `53ecec7` ingest recovery)

- Prefer tip SoT runbook [`docs/qa/PI-APPLIANCE-7.0.md`](qa/PI-APPLIANCE-7.0.md) + [`docs/AUDIT-2026-08-26.md`](AUDIT-2026-08-26.md).
- After hub YAML / map changes: `audit_hub_ingest.py --critical-only` + `verify-brain`; expect SPA `index-BXZwXQYP`.
- Residual code (not docs): alias `_HUB_SWITCH_ENTITY_TO_OID`; add `grow_mat_demand` to `appliance_driver.DEMAND_TO_SEAT`.
- Prefer this tip’s docs PR for SoT; close/supersede #89–#96 after review if overlapping.
- Do **not** invent height/chem/PPFD/NPK; do not paste secrets into Wiki/PRs.
