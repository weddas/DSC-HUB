...
| Item | Status | Notes |
|------|--------|-------|
| Developer SoT `docs/brain/HONESTY.md` | **done (docs)** | Tip `6230383` → `5db15bb` — dual-home, Want/Need, binary bus, Root NPK/Rate on `readingOk`, `got_*` history, coldest-root trust skip, `got_ph` fleet map. |
| WEBUI / DECISION_LOOP / brain README / DSC-BRAIN | **done (docs)** | Cross-links + SPA honesty surfaces + coldest-root / `got_ph` pitfalls. |
| Notion tip blurbs | **done (docs)** | Engineering Ops · Pi offline brain · Local webserver UI (tip `5db15bb`). |
