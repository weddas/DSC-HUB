...
| Live help 1.2.3 | **pending verify** | Public tunnel 502; WP LAN `10.10.10.13` still down — publish blocked (`7fa85b5`) |
| Pi SPA deploy polish | **done** | Hot-patched `index-Ciw7XTuZ.js` to `.48:8787` after syncing C:→Y: spa-dist (deploy packs NAS tree) — tip `7fa85b5` |
| Docker prebuilt seed missing | **noted** | `deploy-brain.ps1` tarball omits `brain/data/demo-fleet-seed.json` while `Dockerfile.prebuilt` COPYs it → compose build fails → hot-patch path; seed synced onto Y: — see [`ops/DSC-HUB-DOCKER.md`](ops/DSC-HUB-DOCKER.md) |
