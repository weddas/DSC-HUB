...
Operators land on Overview / Climate / Grow / Fleet and see **Age / HS·HB / RF**, Want/Got colour, and desk-specific jargon. Raw floats (`Age 20402.7890625`), heartbeat ticks formatted as hours, Up-while-offline chips, and unexplained greys were audit debt (**DA-P1-1**). Tips `39d7f88` → `a50d402` close that path with:
...
4. Honesty rail names **OOS pots**; Dash pot chips split **ESP-NOW radio** vs **Modbus soil probe** (`adbd5c1`)
5. Committed `spa-dist` rebuild (`a50d402`) so operators get HelpTip / honesty without a studio-only rebuild
...
| Honesty rail | `Honesty.tsx` + `sensorHonesty.ts` | ≤6 chips + `+N` overflow; OOS pot count; CTAs avoid demoted Mission; CFM nameplate → Learning |
| Dash ESP chips | `DashHomeSections.tsx` `DashEspLinkChips` | Per-pot ESP-NOW vs Modbus probe labels + tooltip; click → Root |
...
## HubLinkLine Age / Beat honesty (tip `deafc88` → `a50d402`)
...
## ZoneFocus URL ownership (tip `3b082e5` → `a50d402`)
...
## Honesty rail + held Got (tip `8fc5e33` → `adbd5c1`)
...
| OOS pots | `isPotInService` false for pot 1–4 → muted gap `Pot N OOS` / `N pots OOS` → `/live/root` (Live still omits fake Got) |
...
```mermaid
flowchart TB
  inv["input_boolean.dsc_potN_in_service"] --> seat[isPotInService]
  seat -->|false| gap["HonestyGap oos-pots"]
  gap -->|cta| root["/live/root"]
  live[Live Got surfaces] -->|skip OOS pots| omit[No fake moisture]
```

## ESP-NOW vs Modbus pot chips (tip `adbd5c1`)

`DashEspLinkChips` (Dash / demoted home) used to say `P{n} direct|fallback` from ESP-NOW alone — operators treated dead Modbus probes as radio faults.

| Label | Condition | Meaning |
|-------|-----------|---------|
| `P{n} ESP+probe` | ESP on + Modbus entity on | Radio hop and soil bus both healthy |
| `P{n} ESP · probe dark` | ESP on + Modbus known off | **Not an ESP failure** — soil probe offline |
| `P{n} ESP` | ESP on + Modbus entity absent | Radio up; no Modbus binary present |
| `P{n} Modbus` | ESP off + Modbus on | Fallback path — probe still answering |
| `P{n} probe dark` | ESP off + Modbus known off | Both paths dark |
| `P{n} link dark` | ESP off + Modbus entity absent | No radio; Modbus binary missing |

Entities: `binary_sensor.dsc_hub_pot{n}_esp_now_link` · `binary_sensor.dsc_pot{n}_modbus_probe_online`. Chips navigate to Root; `StatusChip` `title` carries the operator tooltip.
...
## Developer pitfalls (verified tip `a50d402`)
...
| Pot link chips | Treat ESP-NOW alone as “probe health” | Split ESP radio vs Modbus probe (`DashEspLinkChips`) |
| OOS pots | Silent omit with no rail cue | Muted honesty gap naming pot numbers → Root |
...
- Grey **RF** / OOS greys are not always faults — inventory out-of-service stays quiet on purpose; honesty rail still **names** OOS pots.
- Do **not** invent catalog height / chem / PPFD / NPK — Research tip states blanks stay blank.
- **spa-dist shipped on tip `a50d402`:** entry `index-Ciw7XTuZ.js` (+ `tune-fleet-KoWKiPmD` · `calibrate-CzxOXAJy` · CSS `index-FUwb4k_E`). HubLinkLine/`dsc-help-tip` land in the tune-fleet chunk; OOS + ESP/Modbus strings in the index chunk. Tip `7fa85b5`: same hashes **hot-patched** to `.48:8787` after C:→Y: spa-dist sync. After further SPA edits, rebuild + sync to Y: + commit hashes or omit `-SkipSpaBuild` — see [`../ops/DSC-HUB-DOCKER.md`](../ops/DSC-HUB-DOCKER.md).

## Residual (next polish)

| Gap | Notes |
|-----|-------|
| PD Help 1.2.3 live verify | WordPress-PD `1064b0d`; tunnel 502 + WP LAN `10.10.10.13` down — re-check when up ([FOLLOWUPS](../FOLLOWUPS.md)) |
| Docker image-build seed pack | `deploy-brain.ps1` omits `demo-fleet-seed.json` → compose build fails → hot-patch ([`DSC-HUB-DOCKER.md`](../ops/DSC-HUB-DOCKER.md)); Pi SPA polish already live via hot-patch (`7fa85b5`) |
