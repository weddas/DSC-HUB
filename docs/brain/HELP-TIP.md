...
- **spa-dist shipped on tip `a50d402`:** entry `index-Ciw7XTuZ.js` (+ `tune-fleet-KoWKiPmD` · `calibrate-CzxOXAJy` · CSS `index-FUwb4k_E`). HubLinkLine/`dsc-help-tip` land in the tune-fleet chunk; OOS + ESP/Modbus strings in the index chunk. After further SPA edits, rebuild + commit hashes or omit `-SkipSpaBuild` — see [`../ops/DSC-HUB-DOCKER.md`](../ops/DSC-HUB-DOCKER.md).

## Residual (next polish)

| Gap | Notes |
|-----|-------|
| PD Help 1.2.3 live verify | WordPress-PD `1064b0d`; tunnel/LAN intermittent — re-check when up ([FOLLOWUPS](../FOLLOWUPS.md)) |
| Pi deploy still gated | Polish gates recorded in tip `f382900` — do not claim live Pi until deploy unblocked |
