...
**Tip (v7.4.0 signed off):** `32836fe` · SPA `index-K2_ziUnM.js` · MP-043 + MP-040 verify
...
- Multi-sensor `*_b` roles and `leak_floor_2x4` remain deferred — add **one** device type at a time. MP-040 `leak_floor_4x8` closed on tip `f029702` lab evidence (still current at `32836fe`).
