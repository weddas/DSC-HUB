...
**Intent:** On the Pi SPA, Add-as-Plant / Compose must choose a tent and derive grow stage from sprout date so the 2×4 lighting/climate path can follow a real plant. Verified against tip `65d4104` (`stage_model.py`, `compose_ops.py`, `control_ops.apply_clone_tent_automation`, `ComposePlant.tsx`).
