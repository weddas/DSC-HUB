...
**Tip (v7.4.0 signed off):** `32836fe` · SPA bundle `index-K2_ziUnM.js` (+ `calibrate-BqnIG9Rc.js` · `tune-fleet-C9fzhOX5.js` · `twin-three-BjdbWAdH.js`)
**Prior (Post-mega D-C-A-B):** `f029702` · `index-CXq-NptO.js`  
**Prior Mega Pass tip:** `a307dc7` (`index-DlMHgtYz.js`)  
**Prior audit tip:** `a2f5f08` / `3a452f0` (`index-D7pAmjOB.js`)  
...
**Code:** `compose_store.ROSTER_SLOT_COUNT` · `compose_ops.assign_to_pot` / `retire_roster_slot` · `plant_probe.release_conflicting_slot_pots` · `api.POST /roster/slots/{n}/retire` · SPA `PlantWizard` + `plantWizard/*` · `GrowPages` + `RosterLifecycleDialogs` (`vacantProbes`, `confirmRetire`) · `seatModel.buildPlantSeat` · `useBrain.refreshComputed` · `fleetApi.get_fleet_computed` · `ui.flushEntityTextDrafts` · `composePlantLogic.clearComposeDraft` · `brain/tests/test_roster_stress.py`  
**Rules:** [`.cursor/rules/dsc-roster-probe.mdc`](../../.cursor/rules/dsc-roster-probe.mdc) · [`.cursor/rules/dsc-pi-hotpatch.mdc`](../../.cursor/rules/dsc-pi-hotpatch.mdc) · [`.cursor/rules/dsc-kit-sot.mdc`](../../.cursor/rules/dsc-kit-sot.mdc)  
**Trail:** [`../FOLLOWUPS.md`](../FOLLOWUPS.md) § *Post-mega D → C → A → B* · § *Operator kit scope* · [`../qa/AUDIT-CLOSURE-2026-09-D-C-A-B.md`](../qa/AUDIT-CLOSURE-2026-09-D-C-A-B.md) · [`../ops/PI-HOTPATCH.md`](../ops/PI-HOTPATCH.md)  
...
Related lifecycle (detach/assign/move): [PLANT-PROBE-LIFECYCLE.md](PLANT-PROBE-LIFECYCLE.md) · wizard steps: [PLANT-WIZARD.md](PLANT-WIZARD.md) · module map: [SPA-MODULE-MAP.md](SPA-MODULE-MAP.md) · kit: [KIT-SCOPE.md](KIT-SCOPE.md)

## Intent

Operators stage more plants than kit probes (Probe 1–2 live). Stock commits fill tent calendars and Keep recipes until a probe is free. Expanding 8 → **10** slots matched a browser stress pass that creates eight 4×8 stock plants plus two seated plants. Tip `3a452f0` closed audit gaps that left identity / drawer state lying after Delete. Mega Pass `a307dc7` closed sprout/stage W— on assign (MP-005) and vacant-probe strip drift (MP-012).
...
## Sprout / stage on assign (Mega Pass MP-005)

Commit+assign must carry sprout onto **both** the pot recipe helpers and the roster slot — otherwise Roster / CropScheduler show `W—` while Compose Review looked correct.

| Layer | Behavior (tip `a307dc7`) |
|-------|--------------------------|
| Brain `assign_to_pot` | Writes pot `datetime.dsc_probe{n}_sprout_date` from recipe; `update_roster_slot(..., sprout=recipe.sprout_date[:10])` |
| SPA `buildPlantSeat` | Prefers pot datetime; **falls back** to `roster.sprout`; derives days via `daysSinceSproutIso` when sensor missing; stage falls back to `select.dsc_probe{n}_growth_stage` |
| Test | `test_commit_and_assign_carries_sprout_to_pot_and_slot` in `test_roster_stress.py` |

```mermaid
flowchart LR
  recipe["compose recipe.sprout_date"] --> pot["datetime.dsc_probeN_sprout_date"]
  recipe --> slot["roster slot.sprout"]
  pot --> seat["buildPlantSeat"]
  slot -->|"fallback if pot empty"| seat
  seat --> days["daysSinceSproutIso"]
```

Do **not** invent stage bands client-side when sprout is missing — show honest empty / `—`.

## Vacant probe strip (Mega Pass MP-012)

`GrowRosterPage` `vacantProbes` = kit probes that are in service, have **no** plant name helper, and are **not** claimed by any roster slot `pot=N`.

| Before | After (`a307dc7`) |
|--------|-------------------|
| Name-empty alone → “vacant” | Also exclude roster `pot` claims |
| Strip could offer Probe 1 while a stock→assign race still claimed it | Assign modal / strip stay aligned with roster SoT |

Constraint: kit chrome still uses `KIT_PROBE_NUMBERS` (1–2), not pot 3/4.
...
- **Sprout W— after assign** — fixed tip `a307dc7` (slot sprout + seat fallback); re-test with `test_commit_and_assign_carries_sprout_to_pot_and_slot`.
- **Vacant strip vs roster claim** — strip must exclude claimed pots (MP-012); do not trust name-empty alone.
...
`test_roster_stress.py` covers: 10-slot default + 8→10 migrate, full-roster commit raise, stock commit without probe, stock/active retire (incl. **`plant_uuid` clear**), empty retire raise, occupied-probe assign reject, stale claim release, API retire + out-of-range, **commit+assign sprout on pot+slot** (MP-005).
