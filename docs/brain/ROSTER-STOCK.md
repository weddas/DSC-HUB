...
**Tip:** `15d7016` · SPA bundle `index-Bx0-MSV-.js` (+ `calibrate-CadBxMGV.js` · `tune-fleet-DouQtSz_.js`)  
**Code:** `compose_store.ROSTER_SLOT_COUNT` · `compose_ops` · `plant_probe.release_conflicting_slot_pots` · `api.POST /roster/slots/{n}/retire` · SPA `PlantWizard` · `GrowPages` · `CropScheduler` · `ui.flushEntityTextDrafts` · `composePlantLogic.clearComposeDraft`  
...
- **Hard-reload after slot retire** — SPA table can lag the API (“already empty” on double-delete). Prefer `refreshBrain` + reload if the row sticks (ST-P0-7).
- **Skip light** before Next on the Light step or the wizard stalls waiting for a fixture pick.
- **Rapid catalog → commit** without bus sync can commit the previous strain; tip includes `syncComposeTextToBus` — still wait for helpers if automating.
...
Browser recipe (Compose stock): coordinate-click catalog hits; JS-click primary Next; confirm modal **Add to Roster stock (no probe)**; hard-reload Roster after retire. Script sketch: `.audit/browser-compose-stock.py`.
