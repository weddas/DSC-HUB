...
| SPA bundle (spa-dist) | `index-DL1EcjhX` (+ `tune-fleet-IPnSFs3d` · `calibrate-D1D5CnxU`) — **source** tip has HelpTip / Age format; **rebuild spa-dist** before operators see it |
...
## Hub link chrome (Age / Beat / HelpTip)

`HubLinkLine` mounts on Dash, Mission, and Fleet/Tune. **Age** and **Beat** prefer HA age entities when present, else fleet uptime/heartbeat **seconds**, always through `fmtUptimeSeconds` — never raw floats. Inline `?` uses native `<details>` (no help JS). Full contract + rebuild note: [`HELP-TIP.md`](HELP-TIP.md).
