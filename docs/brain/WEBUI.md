...
## Climate honesty (tip `6f1b1fa`)

Operator Climate must match real air math — no theater chips.

| Surface | Honesty rule |
|---|---|
| Air path (`AirPathMap`) | Lane schematic + recirc under the diagram; cascade ≠ intake 2×4 |
| Pressure chips | Intake / exhaust **capacity** totals + `sensor.dsc_flow_net_pressure_cfm` (= capacity in − capacity out). Positive = over-pressure. Do **not** verdict mass from the allocated CFM pair (mirror math) |
| Flow Sankey | CFM edges only; `massBalanceOk={null}` (no invented OK/imbalance chip) |
| Efficacy | `sensor.dsc_bought_runtime_today` = **sum of appliance-hours** (overlapping, not wall-clock); lung heat = `sensor.dsc_vent_heat_transfer_btu` (recirc × measured tent−room ΔT) — **not** an outdoor dump sensor |
| Tent air-temp band | Hub **8.1.0.0** publishes stage temp min/max; SPA `TentTargets` disables with *needs hub 8.1.0.0* on older hubs. Band is **inert for the ladder** this train (`stage_temp_target` remains the sole ladder input). Follow 4×8 **mirrors** 4×8 entities |

```mermaid
flowchart TB
  CapIn[intake fan-curve capacity] --> Net[net_pressure = in - out]
  CapEx[exhaust fan-curve capacity] --> Net
  Alloc[allocated CFM pair] -.->|do not use for mass verdict| X[mirror math]
```

## Photoperiod re-anchor (not a shift plan)

Store is **lights-on clock + want hours**; lights-off is derived. `ScheduleOverride`
(on Light / CropScheduler) lets the operator edit either end; the other is
recomputed and previewed, then written via `time.set_value` on the hub
(`control_ops._hub_time` → native API `time_command`).

```mermaid
flowchart LR
  On[lights_on_time] --> Window[on + want_hours = off]
  OffEdit[operator edits off] --> RecomputeOn[recompute on]
  RecomputeOn --> TimeSet[time.set_value hub]
```

Constraints:

- Immediate re-anchor for “match reality tonight” — **not** the gradual path
  (`schedule_shift.py` approve-only 15–30 min/day plans; still require `confirm=true`).
- 2×4 Follow 4×8 disables the control with an honest reason (re-anchor 4×8, or set
  Independent on Light).
- Want hours stay fixed for the stage; this control does not change the rail.
