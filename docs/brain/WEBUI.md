...
**Tip (post-stress audit):** `a2f5f08` / `3a452f0` · spa-dist `index-D7pAmjOB.js` (+ `calibrate-BBqbkhla.js` · `tune-fleet-BdNG_Y6Z.js`) · [ROSTER-STOCK.md](ROSTER-STOCK.md) · [PLANT-WIZARD.md](PLANT-WIZARD.md) · [PLANT-PROBE-LIFECYCLE.md](PLANT-PROBE-LIFECYCLE.md)  
...
| `#/grow/roster` | Plant roster — detach / assign / **slot Delete** (drawer close via captured `retirePot`) |
...
Windows lab path: [`../ops/PI-HOTPATCH.md`](../ops/PI-HOTPATCH.md) (`pscp`/`plink`, not OpenSSH `scp`). Verify served `index.html` hash matches spa-dist (`index-D7pAmjOB.js` on tip).
...
- Verify: `pytest brain/tests/test_roster_stress.py -q`
