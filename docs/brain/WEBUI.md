...
**Tip:** `42a4f92` — plant/tent energy/journals **SHIPPED in repo** · spa-dist `index-DMMGdgxP.js` (+ `calibrate-BsD2unIH.js` · `tune-fleet-BLiBruAo.js` · `twin-three-BjdbWAdH.js`)  
Pi hotpatch + live walk still pending. Room + DSC-Core journals **ABSENT** (closure design approved on tip). Prior surface body: v7.4.0 `32836fe` / `index-K2_ziUnM.js`.

## Surfaces (product SPA)

| Route | Job |
|---|---|
| `#/ops/home` | Ops overview (vitals, honesty, grow-log consumer) — **planned** primary host for room + DSC-Core journals |
...
flowchart TB
  ...
  energy["/energy/* /journal/plant|space"] --> panel[LightEnergyPanel + tent/plant journals]
```
...
## Space / energy / journals (SHIPPED plant→tent — tip `a22050a`+)
...
| HTTP | `/journal/plant/*`, `/journal/space/*`, `/spaces`, `/energy/*` — shift plan requires `confirm=true` |
...
**Room + DSC-Core (ABSENT):** no `room_model` / `room_journal` / `dsc_core_journal`; no `/rooms` or `/journal/core`. Primary UI planned on **Ops/Dash** per [Pi closure design](../superpowers/specs/2026-09-01-space-energy-pi-closure-design.md). Tent journals remain on Light.
...
- Claiming room / DSC-Core journals live before modules + routes exist
