...
> **Superseded (DSC-HUB 7.0.0):** Bridge removed from the Pi product path. DSC-Brain drives Sonoff relays via Native API (`brain/dsc_brain/appliance_driver.py`). From tip `88a4faa`, demand is read from the **ESPHome ingest snapshot** (`hub.values["controls"]`) — not a second hub Native API session every 2 s. Host serialisation and failsafe freshness: [CONCURRENCY.md](CONCURRENCY.md). Firmware below is lab archaeology only.
