...
Verified tip: `9343be6` / `516a6b3` (+ probe rename train `654d0f8`; OTA ops closed `05e8471`) against `climate_mode.py`, `follow_plants.py`, `control_ops.apply_clone_tent_automation`, hub `apply_clone_mode`, panel 0xD1 v2.
...
Live kit (FOLLOWUPS): clone_mode **Follow Plants**.
...
- Probe entity ids are `dsc_probeN_*` (`654d0f8`); live boards OTA’d hub+probe1/2+control @ 7.0.0.0 — see [`ESPHOME-OTA-PI.md`](../ops/ESPHOME-OTA-PI.md).

## Residual

- Follow Plants still keys 2×4 via roster tent + name/strain (firmware `text.dsc_probeN_assigned_plant_id` exists; Follow Plants job not fully rewired to it yet).
