...
**Tip:** `4817c47` (seed packed for image builds) · surface still **7.3.0** / demo image tag `7.3.0-demo` until Phase E bumps `7.4.0`.
...
- Store Cloudflare tunnel tokens in Notion credentials — never commit.

## Seed + image-build

- Seed file: [`brain/data/demo-fleet-seed.json`](../../brain/data/demo-fleet-seed.json) (`version` `7.0.0.0-demo`, `surface` `7.3.0-demo`).
- Simulator resolves `REPO_ROOT/brain/data/demo-fleet-seed.json`, then falls back to package-relative `data/demo-fleet-seed.json`; missing seed → `FileNotFoundError`.
- `Dockerfile.prebuilt` **COPY**s that path into the image; live Pi `deploy-brain.ps1` (tip `4817c47`) packs `data/demo-fleet-seed.json` into the upload tarball and throws if the NAS tree lacks it.
- Demand switches map to simulated relays (`heater` / `humidifier` / `dehumidifier` / `heatmat`).
- Appliance driver and Native API clients are **not** started.
