...
**Tip:** `39233ae` (spa-dist `index-K2_ziUnM.js`, same as v7.4.0 `32836fe`) · QA walk [`docs/qa/LIGHT-AUDIT-7.4-live.md`](../qa/LIGHT-AUDIT-7.4-live.md)
...
## Ownership today vs approved design

**Today (SHIPPED):** tent helpers + stage automation own the window. `control_ops.apply_clone_tent_automation` and `PlantSeatPanel.applyTent` can write 2×4 photoperiod from plant/seat context. Climate Mode Follow Plants writes `clone_*` climate bands — not a substitute for space-owned lighting.

**Approved design (DESIGN ONLY):** photoperiod is **space-owned**; slides/ramps require explicit confirm; plant/space journals and energy TOU calculator are planned — see:

- Design + verified appendix: [`../superpowers/specs/2026-09-01-space-energy-journal-design.md`](../superpowers/specs/2026-09-01-space-energy-journal-design.md)
- Implementation plan: [`../superpowers/plans/2026-09-01-space-energy-journal.md`](../superpowers/plans/2026-09-01-space-energy-journal.md)

Do **not** document tariff, plant journal, or approve-only ramp as live until those modules exist.
...
- Flat `GET /grow-log` is the only journal-like surface today — not plant/space journals.
