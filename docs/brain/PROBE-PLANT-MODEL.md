...
Verified tips: `654d0f8` (rename / `assigned_plant_id` / `cal_session`) + `18849da`/`05e8471` (kit probe1+2; Expected-stage chips; ops closed). Against `firmware/v4/dsc-pot-common.yaml`, hub providers, SPA `kitInventory.ts` / Plant\* chips, brain `dash_computed.py` / `sensor_trust.py`. **Live OTA:** hub + pot1 + pot2 + control @ 7.0.0.0 — see [`PROBE-RENAME-CLEANUP.md`](../qa/PROBE-RENAME-CLEANUP.md) and [`ESPHOME-OTA-PI.md`](../ops/ESPHOME-OTA-PI.md).
...
**Secrets stay:** `dsc_potN_api_key` / `_ota_password` / `_ap_password` — do **not** rename secret keys. Compose env for brain encrypt must be `DSC_POTN_API_KEY` uppercase — see [`ESPHOME-OTA-PI.md`](../ops/ESPHOME-OTA-PI.md).
...
- Inventory `extra.assigned_plant_id` is also read when present (tip inventory: pot1=`pot1` / Amnesia Blue; pot2/3/4 vacant).
- SPA Root trust (`readPotTrust`): **peer divergence** chip only when `binary_sensor.dsc_peer_mad_alert` is on — never the informational summary sensor string (tip `8b3169c`).
...
SPA chips in Plant Wizard / Plant Seat / Plant Extra show **Expected · …** (muted), derived from sprout calendar / expected-stage sensors.

| Signal | Meaning | SoT |
|--------|---------|-----|
| **Expected · {stage}** | Calendar age model only | Sprout date + `stage_model.expected_stage` → `sensor.dsc_probeN_expected_stage` |
| **Growth stage select** | Operator / roster live stage | Plant Seat select / roster `growth_stage` / `select.dsc_probeN_growth_stage` |
...
### Calendar advance (tip `8b3169c`)

Cold computed (`computed_ops._build_cold_computed_states`) keeps Expected and Growth stage from drifting apart when the operator never touches the select:

```mermaid
flowchart TD
  sprout[sprout_date + auto flag] --> derived["expected_stage days"]
  derived --> expectedEntity["sensor.dsc_probeN_expected_stage"]
  recipe[recipe.growth_stage] --> rankCmp{"stage_rank derived > recipe?"}
  derived --> rankCmp
  rankCmp -->|yes or empty| write["update_pot_recipe growth_stage = derived"]
  rankCmp -->|no — operator later| keep[keep recipe_stage]
  write --> selectEntity["select.dsc_probeN_growth_stage"]
  keep --> selectEntity
```

| Case | Result |
|------|--------|
| No recipe stage, or calendar **ahead** of recipe | Advance recipe + select to derived (e.g. Early → Late Flowering) |
| Recipe **later** than calendar (operator push) | Keep operator override |
| Unknown / non-ranked labels | `stage_rank` → `-1`; calendar can still advance when derived is ranked |

Ranks match select options: Germination → Seedling → Early Vegetative → Vegetative → Late (Push) Vegetative → Early Flowering → Flowering → Late Flowering → Final 48-72h Flowering (`STAGE_RANK` in `stage_model.py`).
...
- Expected chip ≠ Growth stage select ≠ Climate Mode (Expected is age-only; select may be calendar-advanced or operator-later).
- Lowercase compose `dsc_potN_API_KEY` silently empties encrypt keys (`19b5c4b`).
- Station rate/dryback emit (tip `8b3169c`) ≠ stuck alert — stuck stays plant-only ([`../ops/FLEET-SOIL-METRICS.md`](../ops/FLEET-SOIL-METRICS.md)).

## Related

- [`CLIMATE-MODE-POLICY.md`](CLIMATE-MODE-POLICY.md) · [`KIT-SOT-SPA.md`](KIT-SOT-SPA.md)
- [`SOFT-CAL.md`](../ops/SOFT-CAL.md) · [`ESPHOME-OTA-PI.md`](../ops/ESPHOME-OTA-PI.md) · [`../ops/FLEET-SOIL-METRICS.md`](../ops/FLEET-SOIL-METRICS.md)
