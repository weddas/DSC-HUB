# SPA viz — Apache ECharts

**In one line:** Live gauges, history lines, sparklines, and Climate FlowSankey render through a tree-shaken ECharts canvas wrapper — held/stale honesty, no invented CFM or mass-balance theater.

**Tip:** `15d7016` (ECharts land `8f4c3e1`; air-only Sankey honesty `28953ae`) · **Operator polish:** [OPERATOR-POLISH.md](OPERATOR-POLISH.md)
...
| `frontend/src/components/FlowSankey.tsx` | **Air CFM only** Sankey over `CfmReading`; heat/humidity estimated splits removed |
| `frontend/src/styles/dsc.css` | `.dsc-echart` / `.dsc-gauge-chart` / `.dsc-sankey-proto` sizing |

**Dependency:** `echarts` `^6.1.0` in `frontend/package.json`.

**SPA tip bundle (spa-dist `15d7016`):** `index-Bx0-MSV-.js` · `tune-fleet-DouQtSz_.js` · `calibrate-CadBxMGV.js` · `twin-three-B0t1gmm4.js`.
...
Mounted under Climate **Air path** card **after** `AirPathMap`.

| Behavior | Reality (verified tip) |
|----------|------------------------|
| Model | Room → 2×4 / 4×8 → Outside / Room recirc (+ cascade) |
| Units | CFM from `resolveCfm` (`cfmProvenance.ts`) |
| Heat / humidity | **Removed** — deprecated props ignored |
| Zero / missing | Links with non-finite or ≤ 0 omitted; empty → “No measured air CFM…” |
| Mass balance | Climate passes `massBalanceOk={null}` — MASS IMBALANCE chip never shown |
| Chip row | `Air CFM` + provenance note (Allocated / Nameplate) |
...
| Sankey as control input | Never — informational CFM only; ladder/brain do not read it |
| `massBalanceOk` | Climate currently passes `null`; do not invent OK/Imbalance |
| `dsc-viz-honesty.mdc` “one CFM surface” | Tip remounts Sankey **beside** Air path; Air path remains SoT CFM — re-verify on Pi before graduating Sankey |
| Heat/humidity Sankey modes | Removed in Wave 4 — do not restore estimated splits |
