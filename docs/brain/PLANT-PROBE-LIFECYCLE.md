...
**In one line:** Assign / move / **detach** keep plant identity on the roster without retire; **stock** parks with no probe; **slot retire** destroys by roster number; SPA Roster / Root / seat drawer are clients of one Assignment SoT.

**Tip (capacity + stock + slot retire):** `15d7016` · SPA `index-Bx0-MSV-.js`  
**Prior Bar 2 tip:** `fe55e4e`  
**Code:** `brain/dsc_brain/plant_probe.py` · `compose_ops.py` · `compose_store.py` · `api.py` · SPA `fleetApi.ts` · `GrowPages.tsx` · `PlantSeatPanel.tsx`  
...
Operators swap probes, free a stick for SoftCal / Soil Test, stage stock for later seating, or park a plant without destroying its recipe. Retire (Delete) is the only destroy path — now **slot-keyed** so stock and detached rows can be removed.
...
| **Slot retire** | `POST /roster/slots/{n}/retire` empties slot; clears probe if this slot owns it | Same as detach |
...
| `POST` | `/roster/slots/{slot_n}/retire` | — | Destroy slot 1–10 |
...
| `#/grow/roster` | Detach · Assign · **Delete** (slot retire for stock/detached/active) |
...
- After slot retire, hard-reload if the table lags the API.
...
Related: [ROSTER-STOCK.md](ROSTER-STOCK.md) · [PLANT-WIZARD.md](PLANT-WIZARD.md) · [WEBUI.md](WEBUI.md)
