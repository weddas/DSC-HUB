...
**Tip:** `6a359f2` · spa-dist `index-BoyhWWR_.js` (+ `calibrate-CYRum8WF.js` · `tune-fleet-BPdxWQzJ.js`)  
**Status:** Pass 1 Light **gate green** · Pass 2 Climate **gate green** · Pass 3 Overview **gate green**. Passes 1–3 program **closed**. **Pass 4 Twin-first gate GREEN** (Tasks 1–7 / prove `4a40589`): Phase A/B/C + full-stress G0–G6; spa `index-BoyhWWR_.js`; index.html sha256 `d00bd5a4be5f2188…`. GPIO5 still **reserved** (not wired; optical N/A). Pass 5 remains stub — offer `/brainstorming` for soak leftovers only.
...
Pass 4 reports: [`.superpowers/sdd/pass4-task-2-report.md`](../../.superpowers/sdd/pass4-task-2-report.md) · [`.superpowers/sdd/pass4-task-3-report.md`](../../.superpowers/sdd/pass4-task-3-report.md) · [`.superpowers/sdd/pass4-task-4-report.md`](../../.superpowers/sdd/pass4-task-4-report.md) · [`.superpowers/sdd/pass4-task-5-report.md`](../../.superpowers/sdd/pass4-task-5-report.md) · [`.superpowers/sdd/pass4-task-6-report.md`](../../.superpowers/sdd/pass4-task-6-report.md) · [`.superpowers/sdd/pass4-task-7-report.md`](../../.superpowers/sdd/pass4-task-7-report.md) ·  
...
| 4 | Twin-first integrated | [`LIVE-UX-PASS4-WALK-2026-09.md`](../qa/LIVE-UX-PASS4-WALK-2026-09.md) (Phase A/B/C + gate filled) | `test_live_ux_pass4_twin.py` | **gate GREEN** on `6a359f2` (`4a40589`) — spa `index-BoyhWWR_.js` |
| 5 | Follow-up | — | — | **stub** after Pass 4 gate |

Screenshots: `docs/qa-screenshots-2026-09-01-live-ux/` (incl. `pass4-b-*` · `pass4-g-*`). Evidence: `.audit/live-ux-light-prove-evidence.json` · `.audit/live-ux-climate-prove-evidence.json` · `.audit/live-ux-overview-prove-evidence.json` · `.audit/live-ux-pass4-prove-evidence.json` (Phase A + **gate**) · `.audit/live-ux-pass4-phaseb-inventory.json` (Phase B).
...
flowchart TD
  ...
  F -->|GREEN 4a40589| P5[Pass 5 stub brainstorm]
```

| Phase | Intent | Verified tip `6a359f2` | Residual |
|-------|--------|------------------------|----------|
...
| C | Clear B + named parks | Task 6 `083c178`; walk C1–C5 **pass**; SV-P1-6 / AirPathMap cascade / GAUGE-P0-1 **fixed**; zigbee/canopy stubs; spa `index-BoyhWWR_.js` | Wet/Dry MQTT + `policy_state` still Pass 5 |
| Gate | Full stress | Task 7 **GREEN** (`4a40589`): G0–G6 pass; twin pytest **12** + focused **22**; live cascade **83.3**; `got_source=twin`; FOLLOWUPS gate section | Pass 5 parks below |
...
| P1 | Canopy / `zigbee_by_role` empty (B6 fail) | **fixed (stubs)** · gate re-proved canopy stub | `_reapply_bindings_to_fleet` safety roles + stubs; gate: Climate Wet/Dry honesty copy mounts; raw Wet/Dry still `—` until MQTT; Problem/Clear needs `policy_state` |
...
**Gate residuals (non-blocking):** historical SF1000 brightness samples are **not** backfilled to `sf1000_on`; Twin Actual `0.0H` with brief OFF cycles can round under 0.1h; energy `confirm=false` saw **422** (Phase B) vs **400** (gate) — both block; Manual Light Hold sticky ON → Pass 5; Wet/Dry MQTT prove → Pass 5.
...
**Operator handoff (gate):** Hub **GPIO5** = reserved Twin SF1000 PWM module — software treats Twin as live actuator (`got_source=twin` at gate); never claim physically wired / optical until operator completes GPIO5 rig-up. Durable note: `docs/FOLLOWUPS.md` Pass 4 gate section.
...
.audit/live-ux-pass4-prove.ps1   # PASS4_PHASE=GATE — Pass 4 gate GREEN
```
...
- Pass 4 **gate GREEN** (`4a40589`) — do not invent Pass 5 scope; offer `/brainstorming` for parks only.
