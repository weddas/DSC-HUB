...
Prefer **`docker stop` + `docker start`** for brain after Hold-map hotpatch. Task 5 `docker kill` hung the Pi; gate recovered cleanly with stop+start. `policy_state` clears on restart until MQTT evaluate — re-seed dry pub if asserting Problem/Clear.
...
- Brain container: prefer **`docker stop` + `start`** (Pass 5); avoid bare `docker restart` and prefer not to `kill` (Task 5 hang).
