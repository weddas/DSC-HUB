# DSC-HUB v2.4.1 — Changelog

*(19 July 2026 — urgent control fixes from the same-day incident, plus
HA-drop resilience, sensor efficiency, and the ESP-NOW link going live.)*

## 1 · Control fixes (the 19 Jul incident)

### 1.1 Fans-first enforced for the AC
"Appliances only after fans failed" was assumed, never enforced — at
5:56 pm the AC fired for the 2×4 while the exchange fans idled. The AC
rung now requires the arbitrated tent's fan lever to be **demonstrably
saturated** (4×8 → OUT ≥ 80%; 2×4 → the router's `clone_fan_saturated`
flag: cooling flush requested AND the cap/budget ceiling reached) — or
**physically useless** (room air ≥ target + 1 °C: venting can't cool
below source temperature), or the room sensor blind, or the fan curve
not running (manual/standby). Supporting changes: graded 2×4 cooling
escalation (exchange 50/75/100% by °C over target), chain-service OUT
lift permitted for an overheating 2×4 even with the 4×8 under target
(ceiling 60%), and the clone cap's RH leg relaxes to a 50% floor during
emergency cooling of the 2×4 itself (the cold leg never relaxes).

### 1.2 VPD-aware humidifier rungs
Both humidifier rungs fired on the RH floor only — VPD (the variable
the strategy chases) could sail over its ceiling with the humidifier
silent (5:28 pm: VPD 0.94/0.99 over ceiling, RH 63%, no response).
Under the VPD strategy "too dry" is now **either** the RH-floor breach
**or** arbitrated VPD > ceiling + 0.05. Guards prevent any
humidifier/dehumidifier war: the VPD leg only arms below the
dehumidifier's release point (ceiling − 2) and never while dehumidifier
demand is live; release requires both signals healthy, or RH reaching
its ceiling. Clone rung gets the identical treatment on clone VPD.

### 1.3 Multi-probe root zone + runaway flush
The closed-loop mat sensed **POT1 only** — a cold POT4 (6:07 pm alert)
was invisible. All four probes now feed the mat through a **5–45 °C
plausibility filter** (POT3's faulted flat-0.0 °C probe is ignored —
the exact blocker that parked `min()` in v2.4). The mat heats on the
**coldest** valid pot, cuts when it reaches Mat Root-Zone High, and a
**runaway guard** (hottest valid pot ≥ High + 1 °C) cuts the mat
immediately and flushes the 2×4 with cooler room air — fans handle
runaway heat; the mat only ever adds it. The root-zone soft fault now
means "no pot delivered a plausible reading in 5 min".

### 1.4 Manual Light Hold latch fix
v2.4's ramp engine stamped a blanket 15.5 s light-write-guard on every
15 s tick — the guard was permanently live while the schedule was
armed, so an external SF1000 change could **never** latch Manual Light
Hold and the ramp silently overwrote it (the v2.2 "lights fight me
from HA" bug reintroduced). The guard is now stamped per-write (2 s,
immediately before each light call).

### 1.5 Mid-slew negative-pressure trim
A budget collapse (e.g. the heater interlock cutting OUT while the
intakes were still ramping) could leave the slewed intakes above the
new budget for a tick or two. The slewed values are now trimmed to the
live budget (fresh-air leg first) — negative pressure is a hard
invariant even mid-transition.

## 2 · HA-drop resilience + sensor efficiency

- `api.reboot_timeout: 0s` — the 15-min default **rebooted the hub**
  on every longer HA outage (fans bounce through safe-boot, resume
  prompt arms, ladder timers reset). Local-first device; never again.
- `wifi.power_save_mode: none` — ESP32 modem power-save is the classic
  dropped/half-dead HA API socket cause. Mains-powered; no napping.
- `logger: level: INFO` — DEBUG was streaming every sensor update to
  UART + API log subscribers.
- Publish-rate cleanup (~80% less steady-state API traffic): RSSI
  15 s → 60 s + 3 dB delta; uptime 60 s → 300 s; all 10 ladder
  telemetry sensors keep 10 s responsiveness but gain `delta: 0.5`
  (idle zeros go silent); runtime-tunable numbers poll 10 s → 60 s
  (SF1000 Target keeps 10 s — the OLED dial writes it live).

## 3 · ESP-NOW pot → hub link — LIVE

Direct sensing path, HA out of the mat's loop. All MACs captured
19 Jul (see the inventory in the hub YAML). Verified against ESPHome
2026.6.5: the espnow component is **ESP32-any-framework** — the hub's
arduino build runs it; no ESP-IDF migration needed.

- **Hub:** espnow peers = 4 pot MACs; packet_transport providers
  `dsc-pot1..4` (xxtea via shared `espnow_key`); four internal remote
  sensors; per-pot freshness stamps; four `POTn ESP-NOW Link`
  connectivity binaries (excluded from the alert auto-include).
- **Pots (`dsc-pot-common.yaml` → project version 2.4.1):** unicast
  `soil_temperature` + `soil_moisture` to the hub MAC every 60 s,
  encrypted.
- **Source-select per pot:** ESP-NOW fresh (<150 s) → HA mirror →
  clone-air rung → mat OFF. Same plausibility filter on every path.

## 4 · Verification

- `esphome config` clean on hub (arduino) and pots (esp-idf),
  ESPHome 2026.6.5.
- QA rig (`qa/qa_rig_v241.cpp`) extended to nine suites — grid sweep,
  boundaries, fuzzer, v2.3 regression, 19 Jul incident replays,
  photoperiod/ramp engine, ESP-NOW source-select:
  **24.29 M checks, 0 violations.**
- Base verified hunk-by-hunk against the canonical v2.4 files.

## 5 · Entity changes

**New:** `binary_sensor.dsc_hub_pot1_esp_now_link` … `_pot4_…`
(connectivity, diagnostic). **No renames, no retirements** — dashboards
and automations keep working unchanged.
...
- **POT1 only** for Alpha — a `min()` across pots waits on POT3, whose faulted
  ...
  *(v2.4.1: closed — plausibility filter + all four pots; see the v2.4.1 section.)*
...
- ESP-NOW consumer block **parked** in-file (pot MACs + shared pot package are
  ...
  *(v2.4.1: LIVE — see the v2.4.1 section.)*
...
| ESP-NOW direct node→hub links | Parked block; needs pot MACs + shared pot package first. *(v2.4.1: LIVE.)* |
...
1. `qa_rig.cpp` Suites 1–6 update — they still assert Clone LED Demand flips
   ...
   *(v2.4.1: closed — `qa/qa_rig_v241.cpp`, nine suites, 24.3M checks.)*
