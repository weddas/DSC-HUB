...
*(v2.4.1: see §11 at the end for the incident-driven judgement calls; several
"open" items below are now closed and marked so.)*
...
*(v2.4.1: SUPERSEDED — all four pots now feed the mat through a 5–45 °C
plausibility filter; POT3's faulted probe is ignored rather than feared. The
reasoning below stands as the record of why v2.4 shipped single-pot.)*
...
*(v2.4.1: no longer true — ESP-NOW carries the probes directly; an HA restart no
longer touches the mat's sense loop.)*
...
`*_uptime`, etc.). *(v2.4.1 helps here: the telemetry delta filters cut recorder
churn at the source — see changelog §2.)*
...
*(v2.4.1 addendum: the 19 Jul alerts printed "below its 0% floor" and "exceeded
its unavailable kPa ceiling" — the alert templates read clone threshold entities
that returned `unavailable` and defaulted to 0. Guard every threshold read with
`| float(default)` against a real number, and point them at the current
`number.dsc_hub_clone_*` entities.)*
...
*(v2.4.1 leans on the same mechanism from the other side: the four POTn ESP-NOW
Link sensors are `connectivity` class precisely so a link blip never pages anyone.)*
...
| **Scribe triple-reporting** | 🟡 Annoying | §5. Retune first, then fix #1 (+ the v2.4.1 threshold-guard addendum). |
| **Pot entity_id inconsistency** | 🟡 Trap for future-you | `install-instructions.md` §6. |
| **POT3 probe swap-test** | 🟡 Hardware | ~~Blocks mat `min()`~~ *(v2.4.1: no longer blocks anything — the plausibility filter ignores it. Still worth fixing the probe.)* |
| **`qa_rig.cpp` Suites 1–6** | ✅ Closed in v2.4.1 | `qa/qa_rig_v241.cpp` — nine suites, 24.3M checks, current topology. |
...
| **ESP-NOW node→hub** | ✅ LIVE in v2.4.1 | Both halves shipped; flash order in install §3. |
...
- ~~the QA rig hasn't been updated for the new topology~~ *(v2.4.1: it has)*
...
## §11 · v2.4.1 judgement calls (19 Jul incident + ESP-NOW)

### "Fans first" must be a gate, not a comment

v2.4's ladder said appliances fire "only when the fans have demonstrably failed"
— but nothing *checked*. The comment described an intention the code didn't
enforce, and the 19 Jul AC event walked straight through the gap. The v2.4.1 rule:
**a safety-ordering claim in a comment must exist as a condition in code**, or the
comment is a liability. The AC now literally reads the fan state before it may
fire.

### The control variable must be a ladder input

The strategy chases VPD; the ladder watched RH. Two views of one measurement —
and the ladder watched the wrong one, so VPD could sail over its ceiling with
every appliance silent. If a variable is the *target*, it has to be able to
*trigger*. (With no-war guards, because VPD-high and RH-high can coexist in hot
air, and the answer to that is cooling, not a humidifier/dehumidifier fight.)

### A plausibility filter beats excluding a known-bad sensor

v2.4 excluded POT3 *by name* and therefore shipped POT1-only sensing — which is
how a cold POT4 went unseen. v2.4.1 filters by *value* (5–45 °C): the faulted
probe excludes itself, every healthy probe votes, and a future fault on any pot
is handled without a config change. Exclude behaviours, not serial numbers.

### The mat only ever adds heat; fans do the subtracting

The runaway guard (hottest pot ≥ High + 1 °C) cuts the mat and flushes the 2×4.
Deliberate asymmetry: overheat kills faster than cold, so runaway wins even while
another pot reads cold.

### Blanket write-guards eat the feature they protect

The v2.4 ramp engine stamped a 15.5 s guard every 15 s tick — permanently
suppressing the external-change latch it was meant to protect from self-latching.
Guards must be scoped to the write, not the schedule. (Same class as the stale-
caveat lesson in §2: a protection that outlives its trigger becomes a bug.)

### The hub must never self-reboot because HA went away

`api.reboot_timeout` defaulted to 15 min — every longer HA outage power-cycled
the local-first controller mid-grow. The Sonoffs *keep* their 15-min reboot
(ALWAYS_OFF makes their reboot safe and it doubles as WiFi-stack recovery); the
hub gets 0s because its reboot is never safe and never helpful. Same knob,
opposite answers, both by consequence.

### ESP-NOW is a fallback *chain*, not a replacement

Per pot: ESP-NOW fresh → HA mirror → clone-air → OFF. Every source passes the
same plausibility filter. A dead router, an HA restart, or a pot dropout each
degrade one rung instead of blinding the mat — and the link watchdogs are
`connectivity` class so a blip never pages anyone.
